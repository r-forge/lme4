  ...
  \item The joint distribution of $\bc Y$ and $\bc U$ is Gaussian
    ...
    \begin{displaymath}
      \begin{aligned}
        ...
        &=\frac{\exp(-\frac{1}{2}\|\bm y-\bm X\bm\beta-\bm U\bm u\|^2)}
        {(2\pi\sigma^2)^{n/2}}\;
        \frac{\exp(-\frac{1}{2}\|\bm u\|^2)}{(2\pi\sigma^2)^{q/2}}\\
        &=\frac{\exp(-\frac{1}{2}
          \left[\|\bm y-\bm X\bm\beta-\bm U\bm u\|^2+\|\bm u\|^2\right])}
        {(2\pi\sigma^2)^{(n+q)/2}}
      \end{aligned}
    \end{displaymath}
...
\begin{frame}
  ...
  \item The Cholesky factor, $\bm L(\bm\theta)$ is the
    ...
    \end{enumerate}
    Define $r^2(\bm\theta,\bm\beta)=\|\bm y-\bm X\bm\beta-\bm
    U\bm\mu_{\bc U|\bc Y}\|^2+\|\bm\mu_{\bc U|\bc Y}\|^2$.  Then
    \begin{displaymath}
...
\end{frame}
\begin{frame}
  \frametitle{Penalized sum of squared residuals}
  \begin{itemize}
  \item 
  \end{itemize}
