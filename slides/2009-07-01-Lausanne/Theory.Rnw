...
simplemer <- function(flist, y, X, theta0 = rep.int(1, length(flist)), super = FALSE)
{
...
setPars <- function(rho, theta)
{
    stopifnot(is.numeric(theta),
              length(theta) == length(rho$theta))
    rho$theta <- theta
    Ut <- crossprod(Diagonal(x = rep.int(theta, rho$nlev)), rho$Zt)
    L <- rho$L <- update(rho$L, Ut, mult = 1)
    rho$ldL2 <- as.vector(determinant(L, logarithm = TRUE)$modulus)
    cu <- solve(L, solve(L, Ut %*% rho$y, system = "P"), system = "L")
    rho$RZX <- solve(L, solve(L, Ut %*% rho$X, system = "P"), system = "L")
    rho$RX <- chol(rho$XtX - crossprod(rho$RZX))
    cb <- solve(t(rho$RX), rho$Xty - crossprod(rho$RZX, cu))
    rho$fixef <- as.vector(solve(rho$RX, cb))
    rho$u <- as.vector(solve(L, solve(L, cu - rho$RZX %*% rho$fixef, system = "Lt"), system = "Pt"))
    rho$fitted <- as.vector(crossprod(Ut, rho$u) + rho$X %*% rho$fixef)
    rho$resid <- rho$y - rho$fitted
    n <- length(rho$y)
    rho$ldL2 + n * (1 + log((2 * pi * sum(c(rho$resid, rho$u)^2)/n)))
}
@ 
...
@ 
  \end{center}
  \begin{itemize}
  \item Without permutation, we get the worse possible fill-in.  With
...
\begin{frame}\frametitle{Recap}
  ...
  \item For a linear mixed model, even one with a huge number of
    ...
    $\bm T$ and $\bm S$, then updating $\bm A$,  $\bm L$, $\bm
    R_{\bm Z\bm X}$, $\bm R_{\bm X}$, calculating the penalized
    residual sum of squares, $r$ and a couple of determinants of
    triangular matrices.
