...
\title[lme4]{Mixed models in R using the lme4 package\\Part 3: Longitudinal data}
...
\item From this one plot (Littell et al. do not provide any plots but
  ...
  models) we see that there are differences between locations, not as
  much between blocks within location, and that treatment 2 is
  providing a lower adjusted response.
\end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Even if we suppress the intercept term we still encounter
    redundancies in trying to estimate effects for each level when we
    have more than one categorical covariate in the fixed-effects
    terms.
  \item Because of this we estimate coefficients for ``contrasts''
    associated with the levels of a factor.  If we have $k$ levels we
    generate $k-1$ contrasts.
...
\item Such a situation pass by unnoticed if estimates of variances and
...
\begin{frame}
  ...
  \item When we incorporate too many fixed-effects terms in a model we
    often find out about it because the standard errors of
...
\begin{frame}
  ...
  \item An alternative, which is recommended whenever you have doubts
    about a model fit, is to use \code{verbose=TRUE} (the lines don't wrap and we miss the interesting part here).
  \end{itemize}
<<fm11verb,echo=FALSE>>=
