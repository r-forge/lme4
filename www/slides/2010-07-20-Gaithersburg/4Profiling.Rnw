...
<<sigmaprof,fig=TRUE,echo=FALSE,height=4.5>>=
...
<<sigma1prof,fig=TRUE,echo=FALSE,height=4.5>>=
zeta <- sqrt(qchisq(c(0.5,0.8,0.9,0.95,0.99), 1))
zeta <- c(-rev(zeta), 0, zeta)
...
<<pr8pairs,echo=FALSE,fig=TRUE,height=7.5>>=
print(splom(pr8))
@   
\end{frame}
