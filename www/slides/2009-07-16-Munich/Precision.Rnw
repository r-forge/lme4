...
\institute[U. Wisc.]{
  ...
  {\color{Scode}\texttt{(bates@stat.wisc.edu)}}
}
\date[LMU, 2009-07-16]{LMU, Munich \\ July 16, 2009}
...
\begin{frame}
  ...
  \item The mixed-effects packages for \Rp{} that I have written
    (\pkg{nlme} with Jos\'{e} Pinheiro and \pkg{lme4} with Martin
    M\"{a}chler) do not provide standard errors of variance
    components.  \pkg{lme4} doesn't even provide p-values for the
    fixed effects.
...
\section[Dyestuff]{A simple (but real) example}
...
\begin{frame}
  ...
  \item We never really form $\bm\Sigma_\theta$; we always work with the
    ...
    \end{displaymath}
    so $\theta=\frac{\sigma_b}{\sigma}$ and
...
\begin{frame}
  ...
  \item The key to evaluating the log-likelihood is the Cholesky
    ...
    involves random effects for different factors.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item For this simple model we can evaluate and plot the deviance
    for a range of $\theta$ values.  We also evaluate $\log(|\bm
    L_\theta|^2)$ (\code{ldL2}) and $n\left[1+\log\left(\frac{2\pi
          r^2(\bm\theta)}{n}\right)\right]$ (\code{lprss}).
...
\begin{frame}[fragile]
  ...
  \item For some model/data set combinations the estimate of
    ...
    the increase in the penalty.  The \code{Dyestuff2} data from Box
    and Tiao (1973) show this.
  \end{itemize}
  \begin{center}
...
@ 
  ...
  \item One of the big differences between inferences for $\sigma^2_b$
    and those for $\sigma^2$ is considering what to do about values of
    zero.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
<<profileddevfm1,fig=TRUE,echo=FALSE>>=
...
\begin{frame}
  ...
  \begin{itemize}
  \item The difference of the profiled deviance at a value of $\sigma$
    or $\sigma_b$ is the likelihood ratio test statistic for that
    parameter value.
  ...
  \item If we use the \Emph{signed square root} transformation where
    we assign a sign of $-$ to parameters to the left of the estimate,
