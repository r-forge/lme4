...
\begin{frame}[fragile]
  ...
  \item In the models we will consider, the random effects are modeled
    ...
    $\bc{B}\sim\mathcal{N}(\bm 0,\bm\Sigma_\theta)$, where
...
\begin{frame}
  ...
  \item The crucial calculation in each \code{lmer} iteration is
    ...
    factor, $\bm L_\theta$, derived from $\bm Z$ and
    $\bm\Sigma_\theta$.  Crossing of grouping factors increases
    the number of nonzeros in $\bm L_\theta$ and causes some
...
\begin{frame}[fragile]
  ...
  \item Because both random-effects terms are simple, scalar terms,
    $\bm\Sigma_\theta$ is block-diagonal in two diagonal blocks of
...
\begin{frame}[fragile]
  ...
  \item In this case the \code{x} slot is itself over 1GB in size and
    the \code{i} slot is over 0.5 GB.
