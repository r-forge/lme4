...
\begin{frame}\frametitle{Converting profile $\zeta$ to a density}
  \begin{itemize}
  \item We speak of a profile $\zeta$ plot as showing skewness,
    especially for parameters such as $\sigma_1$ and $\sigma$.
  \item Often it is easier to envision symmetry or skewness in terms
    of a density plot.
  \item If $\zeta$ is compared to a standard Gaussian distribution
...
\begin{frame}[fragile]
  \mode<beamer>{\frametitle{Profile-based densities for \code{fm1}}}
  \begin{figure}[htb]
    \centering
<<pr1dens,fig=TRUE,echo=FALSE>>=
print(densityplot(pr1, layout=c(1,3), strip=FALSE, strip.left=TRUE))
@     
\mode<article>{\caption{Profile-based densities for the parameters in model \code{fm1}}\label{fig:pr1dens}}
  \end{figure}
\end{frame}
