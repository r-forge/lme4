...
\title[Theory of LMMs]{Mixed models in R using the lme4 package\\Part 6: Theory of linear mixed models}
...
\begin{frame}
  ...
  \item We never really form $\bm\Sigma$; we always work with the
    ...
    \begin{displaymath}
      \bm\Sigma_\theta=
      \sigma^2\bLt\bm\Lambda\trans_\theta
    \end{displaymath}
...
\begin{frame}
  ...
  \item The conditional mean satisfies the equations
    \begin{displaymath}
      \left(\bm Z\bLt\bLt\trans\bm Z\trans+\bm I_q\right)
...
\begin{frame}
  ...
  \item Given the grouping factors for the model and a value of
    $\bm\theta$ we produce $\bm Z\bLt$ then $\bm L$, using \code{Cholesky}
  ...
  \item To avoid recalculating we assign
    ...
    \item[\code{Lambda}] current $\bLt$
    \item[\code{Ut}] transpose of $\bm Z\bLt$
    \end{description}
  \end{itemize}
\end{frame}
...
\end{frame}
\begin{frame}[fragile]
  \frametitle{Images of $\bLt\trans\bm Z\trans\bm Z\bLt+\bm I$ and $\bm L$}
  \begin{center}
...
\item Note that there are nonzeros in the lower right of $\bm L$ in
  positions that are zero in the lower triangle of $\bLt\trans\bm Z\trans\bm
  Z\bLt+\bm I_q$.  This is described as ``fill-in''.
\end{itemize}
\end{frame}
\begin{frame}[fragile]
...
\begin{frame}[fragile]
  \frametitle{Checking profDev, bobyqa applied to profDev}
<<profDev1>>=
...
\begin{frame}
  ...
  \item The creation of $\bLt$ is somewhat more
  ...
  \item Once the optimizer, \code{bobyqa}, has converged some
