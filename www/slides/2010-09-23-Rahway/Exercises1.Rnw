...
\usepackage{fancyhdr}
\lhead{\sf Statistics 471}\chead{\sf Exercises \#1}
\rhead{\sf 2010-09-08 (p. \thepage)}
...
names(classroom)
@
\item The variable \code{housepov} should be associated with the
  \code{schoolid} factor. Check that there is only one value of
  \code{housepov} for each school.
\item The variables \code{yearstea}, \code{mathknow}, \code{mathprep}
  and \code{schoolid} should be a property of the \code{classid}.
  Check that they are.
\item Create two new data frames, \code{school} and \code{class},
  containing one row for each level of \code{schoolid} (resp. one row
  for each level of \code{classid}) and the corresponding variable(s)
  that are associated with the school or class.
\item Look up the documentation of the \code{merge} function and see
  if you can merge the two data frames you created in the previous
  question into one frame that contains all the variables associated
  with the class plus the \code{housepov} associated with the class.
\end{enumerate}
\end{document}
