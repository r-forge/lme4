...
this section we briefly describe some of the computational
requirements for generalized linear mixed models, to show why these
more general structures are employed.

\subsection{Definition of GLMMs}
\label{sec:GLMMdef}

The generalized linear mixed models (GLMMs) that can be fit by the
\pkg{lme4} preserve the multivariate Gaussian unconditional
distribution of the random effects, $\mc B$
(\ref{eq:LMMuncondB}).  Because most families used for the conditional
...
the distribution of the spherical random effects, $\mc U$.  That is
\begin{equation}
  \label{eq:UdistGLMM}
...
$\frac{d\eta}{d\mu}$, of such a mapping will be diagonal.)
...
given $\mc Y=\yobs$.  The notation here is a bit blurred because the
joint distribution of $\mc Y$ and $\mc U$ can be (and often is)
discrete with respect to $\mc Y$ but is always continuous with respect
to $\mc U$.  However, when we condition on the observed value $\mc
Y=\yobs$, the resulting function is continuous with respect to $\bm u$
so the unscaled conditional density is well-defined.
...
(\ref{eq:GLMMlike}) by determining the value
$\tilde{\bm u}_{\beta,\theta}$ that maximizes the unscaled conditional
density.  When the conditional density, $\mc U|\mc Y=\bm y$, is
multivariate Gaussian, the conditional mode will also be the
conditional mean.  For most families used in GLMMs the mode and the
mean do not need to coincide so we call $\tilde{\bm u}_{\beta,\theta}$
...
corresponding conditional mean of $\mc Y|\mc U=\bm u$ and $\bm M_i$ is
the Jacobian matrix of the vector-valued inverse link, evaluated at
$\bm\mu_i$.  That is
\begin{equation}
  \label{eq:Jacobian}
  \bm M_i=\left.\frac{d\bm\mu}{d\bm\eta}\right|_{\bm\eta_i},
\end{equation}
