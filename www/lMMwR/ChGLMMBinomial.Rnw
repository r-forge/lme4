...
which the response is \emph{binary},\index{binary
  response}\index{response!binary} in the sense that it represents
...
them and if we specify just the distribution type we get the family
with the canonical link.  Thus our initial fit is generated as
<<fm10,eval=FALSE>>=
fm10 <- glmer(use ~ 1+age+I(age^2)+urban+livch+(1|district),
              Contraception, binomial)
@ 
...
contrasts for a factor such as \code{livch} is offsets relative to
...
they are also close similar to each other.  This confirms our
...
number of children is not an important distinction.
...
in mind that $\mu$ corresponds to a probability for this distribution
...
the odds ratio, $\frac{\mu}{1-\mu}$, is in $[0,\infty)$ and
...
linear predictor scale and not on the probability scale.
