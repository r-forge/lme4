...
fm06 <- lmer(effort ~ 1 + (1|Subject) + (1|Type), ergoStool, REML=0)
...
<<fm06a>>=
fm06a <- lmer(effort ~ 1 + (1|Subject), ergoStool, REML=0)
anova(fm06a, fm06)
@
...
<<fixeffm07>>=
with(env(fm07), head(as.vector(X %*% fixef)))
@ 
...
<<kappafm06>>=
kappa.tri(as(as(env(fm06)$L, "sparseMatrix"), "matrix"), exact = TRUE)
@ 
...
fixed-effects terms (other than the implicit intercept) is called the ``unconditional model'' in the multilevel modeling literature.  We fit it as
<<fm09>>=
