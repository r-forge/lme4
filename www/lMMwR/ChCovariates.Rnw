...
longitudinal data into both the fixed-effects terms and the
random-effects terms of linear mixed models.  In this chapter we will
extend the use of covariates and discuss general approaches to
building and assessing linear mixed models.
...
As we have seen in the earlier chapters, each random-effects terms in
a linear mixed model is defined with respect to a \emph{grouping
...
\citet[Table 23.1]{milliken09:_analy_messy_data} discuss data from an experiment to
measure productivity on a manufacturing task according to the type of
machine used and the operator.  No further details on the experiment
are given and it is possible that these data, which are available as
\code{Machines} in the \package{MEMSS} package, were constructed for
illustration and are not observed data from an actual experiment.
<<strMachines>>=
str(Machines)
...
other studies we may regard the levels of the stimulus as a random
sample from a population of stimuli.

A plot of these data (Fig.~\ref{fig:Machinesplot})
\begin{figure}[tbp]
  \centering
...
\end{figure}
shows high reproducibility of measurements on the same
operator-machine combination.  On each line the scores on a particular
machine are tightly clustered.  There are considerable, apparently
systematic, differences between machines and somewhat smaller
differences between operators except for one unusual combination,
operator 6 on machine B.  The pattern for operator 6 is very different
from the pattern for the other five operators.  We will expect a
significant interaction between the \code{Worker} and \code{Machine}
factors because of this unusual pattern.  As we will see, we can
incorporate such an interaction into a linear mixed model in different
ways.

\subsection{Comparing models with and without interactions}

We fit and compare three models for these data: \code{fm10} without
interactions, \code{fm11} with vector-valued random effects to allow
for interactions, and \code{fm12} with interactions incorporated into
...
\code{fm10}, incorporates three fixed-effects parameters and two
variance components.  Model \code{fm12}, with scalar random effects
for \code{Worker} and for the \code{Machine:Worker} combination
incorporates one additional variance component for a total of six
parameters, while model \code{fm11} adds five variance-component
parameters to those in \code{fm10}, for a total of 10 parameters.

In the comparison of models \code{fm10} and \code{fm12} (i.e. the line
labeled \code{fm12} in the \code{anova} table) we see that the
additional parameter is highly significant.  The change in the
deviance of \Sexpr{sprintf(.f4,deviance(fm10)-deviance(fm12))} (in the
column labeled \code{Chisq}) at a cost of one additional parameter is
huge; hence we prefer the more complex model \code{fm12}.  In the next
line, which is the comparison of the more complex model \code{fm11} to
the simpler model \code{fm12}, the change in the deviance is
\Sexpr{sprintf(.f4,deviance(fm12)-deviance(fm11))} at a cost of 4
additional parameters with a p-value of 
\Sexpr{sprintf(.f1,100*pchisq(deviance(fm12)-deviance(fm11),4,lower=FALSE))}\%.
...
results and here we prefer model \code{fm12}, of intermediate complexity.
