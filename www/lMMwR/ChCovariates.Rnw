...
if (file.exists("pr5.rda")) {
    load("pr5.rda")
} else {
    pr5 <- profile(fm5)
    save(pr5, file = "pr5.rda")
}
fm6 <- lmer(effort ~ 1 + Type + (1|Subject), ergoStool, REML = 0)
if (file.exists("pr6.rda")) {
    load("pr6.rda")
} else {
    pr6 <- profile(fm6)
    save(pr6, file = "pr6.rda")
}
f7 <- "%.7f"
f3 <- "%.3f"
...
In the previous chapter we fit models having different configurations of
...
Indeed, often the purpose of fitting a linear mixed model is to draw
...
exertion and rated type \code{T4} as requiring the most exertion.
...
fit in the same way that we fit models with random effects for
multiple grouping factors in Chap.~\ref{chap:Multiple},
<<fm5>>=
(fm5 <- lmer(effort ~ 1 + (1|Subject) + (1|Type), ergoStool, REML=0))
@
from which we determine that the mean effort to arise, across stool
types and across subjects, is \Sexpr{sprintf(f3, fixef(fm5))} on this
scale, with standard deviations of
\Sexpr{sprintf(f3, attr(VarCorr(fm5)[["Subject"]], "stddev"))}
for the random-effects for the \code{Subject} factor and
\Sexpr{sprintf(f3, attr(VarCorr(fm5)[["Type"]], "stddev"))}
...
comparing this fit to model \code{fm5} using a likelihood-ratio test.
<<fm5a>>=
fm5a <- lmer(effort ~ 1 + (1|Subject), ergoStool, REML=0)
anova(fm5a, fm5)
@
The p-value in this test is very small, indicating that the more
complex model, \code{fm5}, which allows for differences in the effort
...
p-value, \Sexpr{sprintf(f7, anova(fm5a, fm5)[["Pr(>Chisq)"]][2])},
...
reasonable for $\sigma_2$ to be zero is to profile \code{fm5} and
examine profile zeta plots (Fig.~\ref{fig:pr5zeta})
\begin{figure}[tbp]
  \centering
<<pr5zeta,fig=TRUE,echo=FALSE,height=3.5>>=
print(xyplot(pr5, aspect=1.3, layout=c(4,1)))
@   
\caption[Profile zeta plot for the parameters in model \code{fm5}]
{Profile zeta plot for the parameters in model \code{fm5} fit to the
  \code{ergoStool} data}
  \label{fig:pr5zeta}
\end{figure}
and the corresponding profile pairs plot (Fig.~\ref{fig:pr5pairs}).
\begin{figure}[tbp]
  \centering
<<pr5pairs,fig=TRUE,echo=FALSE,height=8>>=
print(splom(pr5))
@   
\caption[Profile pairs plot for the parameters in model \code{fm5}]
{Profile pairs plot for the parameters in model \code{fm5} fit to the
  \code{ergoStool} data}
  \label{fig:pr5pairs}
\end{figure}

We can see from the profile zeta plot (Fig.~\ref{fig:pr5zeta}) that
although $\sigma_2$, the standard deviation of the random effect for
\code{Type}, is safely non-zero, it is very poorly determined.  That
is, a 95\% profile-based confidence interval on this parameter,
obtained as
<<fm5confint>>=
confint(pr5)[".sig02",]
@
is very wide.  The upper end point of this 95\% confidence interval,
\Sexpr{sprintf(f3,confint(pr5)[2,2])}, is more than twice as large as
...
\begin{figure}[tbp]
  \centering
<<fm5dot,fig=TRUE,echo=FALSE,height=2>>=
print(dotplot(ranef(fm5, postVar = TRUE), strip=FALSE)[["Type"]])
@   
...
type]{95\% prediction intervals on the random effects for \code{Type}
  from model \code{fm5} fit to the \code{ergoStool} data}
  \label{fig:fm5dot}
\end{figure}
of the prediction intervals on the random effects for \code{Type}
(Fig.~\ref{fig:fm5dot}) confirms the impression from
...
Typically in an analysis like this we begin by asking if there are any
...
from a model fit such as \code{fm5} in which the stool types are
...
\subsection{Fixed-effects for \texttt{Type}, Random Effects for \texttt{Subject}}
...
from that in model \code{fm5}.  Furthermore, the estimates of the
...
types, are redundant.  If we have a set of estimates for these
...
are subject to estimability conditions.  While this approach may seem
reasonable initially, in complex models it quickly becomes
...
Because these were the contrasts in effect when model \code{fm6} was
...
\begin{figure}[tbp]
  \centering
<<pr6zeta,fig=TRUE,echo=FALSE,height=3.5>>=
print(xyplot(pr6, aspect=1.3, layout=c(6,1)))
@   
\caption[Profile zeta plot for the parameters in model \code{fm5}]
{Profile zeta plot for the parameters in model \code{fm5} fit to the
  \code{ergoStool} data}
  \label{fig:pr6zeta}
\end{figure}
(Fig.~\ref{fig:pr6zeta}) and check, say, the confidence intervals on
these parameters.
<<confintfm6>>=
confint(pr6, c("TypeT2", "TypeT3", "TypeT4"))
@ 
According to these intervals, and from what we see from
Fig.~\ref{fig:pr6zeta}, types \code{T2} and \code{T3} are
...
However, this process must be modified in two ways in two ways to
provide a suitable answer.  The most important modification is to take
into account the fact that we are performing \emph{multiple
  comparisons} simultaneously.  We describe what this means and how to
accomodate for it in the next subsection.  The other problem is that 
this process only allows us to evaluate contrasts of the
reference level, \code{T1}, with the other levels and the reference
level is essentially arbitrary.  For completeness we should evaluate
all six possible contrasts of pairs of levels.
...
\subsection{Fixed-effects for both \code{Type} and \code{Subject}}
\label{sec:ergofe}
