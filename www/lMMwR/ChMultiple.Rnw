...
if (!file.exists("figs/Multiple-fm4Limage.png")) {
    ...
    print(image(env(fm4)$L, sub=NULL, xlab=NULL, ylab=NULL, colorkey=FALSE))
    dev.off()
}
@
...
particular sample.  Hence we will also use random effects for the sample. 
...
@   
  ...
  \texttt{fm2}]{Image of the transpose of the random-effects model
    matrix, $\vec Z$, for model \code{fm2}.  The non-zero elements,
...
\begin{figure}[tbp]
  \centering
<<fm2LambdaLimage,fig=TRUE,echo=FALSE,height=3.5>>=
...
model \texttt{fm2}]{Images of the relative covariance factor,
  $\Lambda$, the cross-product of the random-effects model matrix,
  $\vec Z\trans\vec Z$, and the sparse Cholesky factor, $\vec L$, for
  model \code{fm2}.}
  \label{fig:fm2LambdaLimage}
\end{figure}

The relative covariance factor (Fig.~\ref{fig:fm2LambdaLimage}, left
panel) is no longer a multiple of the identity.  It is now block
diagonal, with two blocks, one of size 24 and one of size 6, each of
which is a multiple of the identity.  The diagonal elements in each
block are $\theta_1$ and $\theta_2$.  The numeric values of
...
Fig.~\ref{fig:fm2LambdaLimage}.  The sparse Cholesky factor, $\vec L$,
...
A profile zeta plot (Fig.~\ref{fig:fm2prplot}) for the parameters in
model \code{fm2}
\begin{figure}[tbp]
  \centering
...
@   
  \caption{Profile zeta plot of the parameters in model \code{fm2}.}
  \label{fig:fm2prplot}
\end{figure}
...
The profile pairs plot (Fig.~\ref{fig:fm2prpairs}) shows
\begin{figure}[tbp]
  \centering
...
@   
  ...
  {Profile pairs plot for the parameters in model \code{fm2} fit to
...
these parameter estimates depending on each other.  It is almost
entirely due to the choice of scale for $\sigma_1$ and $\sigma_2$.  When we
plot the contours on the scale of $\log(\sigma_1)$ and
$\log(\sigma_2)$ instead (Fig.~\ref{fig:toBeConstructed}) they are
much closer to the elliptical pattern.
...
batch effect.  For example, each of batches I and D, with low mean
strength relative to the other batches, contained one sample (I:b and
D:c, respectively) that had high mean strength relative to the other
...
\begin{figure}[tbp]
  \centering
<<fm3LambdaLimage,fig=TRUE,echo=FALSE,height=3.5>>=
...
@   
\caption[Images of $\Lambda$, $\vec Z\trans\vec Z$ and $\vec L$ for
model \texttt{fm3}]{Images of the relative covariance factor,
  $\Lambda$, the cross-product of the random-effects model matrix,
  $\vec Z\trans\vec Z$, and the sparse Cholesky factor, $\vec L$, for
  model \code{fm3}.}
  \label{fig:fm3LambdaLimage}
\end{figure}
...
and $\vec L$ for this model (Fig.~\ref{fig:fm3LambdaLimage}) to those
in Fig.~\ref{fig:fm2LambdaLimage} shows that models with nested
...
(we discuss this reordering later in Sect.~\ref{sec:permutation}).
...
\subsection[Parameter Estimates for Model \code{fm3}]{Assessing the
  Parameter Estimates for Model \code{fm3}}
\label{sec:assessingfm3}
...
$\widehat{\beta_0}=$\Sexpr{sprintf(f3,fixef(fm3))}, the overall mean
...
(Fig.~\ref{fig:fm3ranef}))
\begin{figure}[tbp]
  \centering
<<fm3ranef,fig=TRUE,echo=FALSE,height=6.5,results=hide>>=  
...
@   
  \caption{Profile zeta plots for the parameters in model \code{fm3}.}
  \label{fig:fm3prplot}
\end{figure}
Furthermore, the profile zeta plot (Fig.~\ref{fig:fm3prplot})
...
One of the many famous quotes from Albert Einstein is ``Everything
should be made as simple as possible, but not simpler.''  In
statistical modeling this \emph{principal of parsimony} is embodied in
hypothesis test comparing two models, one of which contains the other
as a special case.  Typically, one or more of the parameters in the
more general model, which we call the \emph{alternative hypothesis},
is constrained in some way, resulting in the restricted model, which
we call the \emph{null hypothesis}.  Although we phrase the hypothesis
test in terms of the parameter restriction, it is important to realize
that we are comparing the quality of fits obtained with two different
models.  
...
to justify the greater complexity of model $H_0$.  This comparison is
...
the model fits, the method for calculating the p-value from the
...
(fm3a <- lmer(strength ~ 1 + 1|sample, Pastes, REML=0)) 
@ 
is compared to model \code{fm3} with the \code{anova} function
<<anovafm3>>=
anova(fm3a, fm3)
@
which provides a p-value of \Sexpr{sprintf(f4,anova(fm3a,fm3)$Pr[2])}.%$
Because typical standards for ``small'' p-values are 5\% or 1\%, a
p-value over 50\% would not be considered significant at any
reasonable level.
...
\subsection{Assessing the Reduced Model, \texttt{fm3a}}
\label{sec:assessReduced}

The profile zeta plots
\begin{figure}[tbp]
  \centering
<<fm3aprplot,fig=TRUE,echo=FALSE,height=3.5>>=
print(xyplot(pr3a, aspect=1.3, layout=c(3,1)))
@   
  \caption{Profile zeta plots for the parameters in model \code{fm3a}.}
  \label{fig:fm3aprplot}
\end{figure}
of the parameters in model \code{fm3a} (Fig.~\ref{fig:fm3aprplot}) are
similar to the corresponding plots in Fig.~\ref{fig:fm3prplot}, as
confirmed by the numerical values of the confidence intervals.
<<confintfm3>>=
confint(pr3)
confint(pr3a)
@
...
wider in model \code{fm3a} than in \code{fm3}, because the variability
that is attributed to \code{batch} in \code{fm3} is incorporated into
the variability due to \code{sample} in \code{fm3a}.
\begin{figure}[tbp]
  \centering
<<fm3aprpairs,fig=TRUE,echo=FALSE,height=6.5>>=
print(splom(pr3a))
@   
  \caption[Profile pairs plot of the parameters in model \code{fm3a}.]
  {Profile pairs plot for the parameters in model \code{fm3a} fit to
    the \code{Pastes} data.}
  \label{fig:fm3aprpairs}
\end{figure}

The patterns in the profile pairs plot (Fig.~\ref{fig:fm3aprpairs})
for the reduced model \code{fm3a} are similar to those in
...
the configuration of the factors is neither nested nor completely
crossed.  We describe such situations as having \emph{partially
...
crossed grouping factors.  When students with scores in different
years have different teachers for the different years, the student
factor is not nested within the teacher factor.  To have complete
crossing of the student and teacher factor is would be necessary for
each student to be observed with each teacher which would be unusual.
A study of thousands of students and hundreds of teachers inevitably
ends up partially crossed.
...
\subsection{The \code{InstEval} Data}
\label{sec:InstEval}

The \code{InstEval} data are from a special evaluation of lecturers by
students at ETH-Z\"{u}rich, to determine who should receive the
``best-liked professor'' award.  These data have been slightly
simplified and identifying labels removed so as to preserve anonymity.
...
xtabs(~ y, InstEval)
@
it is sufficiently spread out to warrant treating is as if it were a
continuous response.

In this chapter we fit models that have random effects for student,
instructor, and department or the \code{dept:service} combination to
these data.  Later we will fit models that have fixed for
...
\begin{figure}[tbp]
  \centering
<<fm4ranef,fig=TRUE,echo=FALSE,height=5,results=hide>>=  
print(dotplot(rr4, strip = FALSE))
@ 
\caption{95\% prediction intervals on the random effects for the
  \code{dept:service} factor in model \code{fm4} fit to the
  \code{InstEval} data.}
  \label{fig:fm4ranef}
\end{figure}
...
(Fig.~\ref{fig:fm4ranef}).  In fact, zero is close to the middle of
...
these terms have practical significance, in addition to the apparent
...
\begin{figure}[tbp]
  \centering
\includegraphics{figs/Multiple-fm4Limage}
\caption{Image of the sparse Cholesky factor, $\vec L$, from model \code{fm4}}
  \label{fig:fm4Limage}
\end{figure}

Before leaving this model we examine the sparse Cholesky factor, $\vec
L$, (Fig.~\ref{fig:fm4Limage}), which is of size $4128\times4128$.
...
crossed or nested as applying to the terms although it is more
accurate to refer to the configuration as applying to the grouping
factors.
...
factors.  The emphasis on hierarchical or multilevel configurations is
an artifact of the computational methods used to fit the models, not
...
large numbers of observations, a term in a model may be
``statistically significant'' even though its practical significance
may be negligible.
