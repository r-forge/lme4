...
@ 
\caption[Random effects prediction intervals for model \code{fm2}]{95\%
...
random-effects terms, as in model \code{fm2}, each term generates a
...
matrix, shown in Fig.~\ref{fig:fm2Ztimage}, contains rows of
...
(Fig.~\ref{fig:fm2LambdaLimage}, left panel) is no longer a multiple
...
@ 
\caption[Random effects prediction intervals for model \code{fm3}]{95\%
  prediction intervals on the random effects for model
  \code{fm3} fit to the \code{Pastes} data.}
  \label{fig:fm3ranef}
\end{figure}
...
$\Sexpr{sprintf("%.4f",anova(fm3a,fm3)[["Pr(>Chisq)"]][2])}$.
Because typical
...
crossing of the student and teacher factor it would be necessary for
each student to be observed with each teacher, which would be unusual.
A longitudinal study of thousands of students with hundreds of
different teachers inevitably ends up partially crossed.
...
identifying labels removed so as to preserve anonymity.
...
'data.frame':	73421 obs. of  7 variables:
 ...
 $ lectage: Ord.factor w/ 6 levels "1"<"2"<"3"<"4"<..: 2 1 2 2 1 1 1 ...
...
@ 
\caption[Random effects prediction intervals for model \code{fm4}]{95\%
