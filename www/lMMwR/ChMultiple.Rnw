...
\chapter{Models with multiple terms}
\label{chap:Multiple}
...
can begin to realize the flexibility and versatility of mixed models.  Also, in practice it is rare not to be considering more than one fixed-effects term.

In this chapter we consider models with multiple simple, scalar
random-effects terms.

\section{Models with crossed random effects}
\label{sec:crossedRE}
...
Fig.~\ref{fig:Penicillindot} and, because there are a moderate
...
block are the $\theta_1$ and $\theta_2$.  The numeric values of
$\vec\theta$ can be obtained as
<<fm2verb>>=
env(fm2)$theta
@ 
...
entirely due choice of scale for $\sigma_1$ and $\sigma_2$.  When we
...
\section{Models with nested random effects}
\label{sec:NestedRE}

In this section we again consider a simple example, this time
illustrating applications of models with \emph{nested} grouping
...
@   
  ...
  \label{fig:fm3prplot}
\end{figure}
is shown in Fig.~\ref{fig:fm3prplot} and the profile pairs plot
\begin{figure}[tbp]
  \centering
...
\end{figure}
in Fig.~\ref{fm3prpairs}.  The contours in the profile pairs plot
