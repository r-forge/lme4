...
library(Matrix)
library(lme4a)
fm03ML <- lmer(diameter ~ 1 + (1|plate) + (1|sample), Penicillin, REML=0)
fm04 <- lmer(strength ~ 1 + (1|sample) + (1|batch), Pastes, REML=0)
fm04a <- lmer(strength ~ 1 + (1|sample), Pastes, REML=0)
...
if (file.exists("fm05.rda")) {load("fm05.rda")
    ...
    fm05 <- lmer(y ~ 1 + (1|s) + (1|d) + (1|dept:service), InstEval, REML=0)
...
\caption[Random effects prediction intervals for model \code{fm03}]{95\%
  ...
  \code{fm03} fit to the \code{Penicillin} data.}
  \label{fig:fm03ranef}
\end{figure}
...
print(image(env(fm03)$Zt, sub=NULL,xlab=NULL,ylab=NULL))
@   
...
print(image(env(fm03)$Lambda, sub=NULL,xlab=expression(Lambda),ylab=NULL),
      split=c(1,1,3,1), more=TRUE)
print(image(tcrossprod(env(fm03)$Zt),sub=NULL,xlab="Z'Z",ylab=NULL),
      split=c(2,1,3,1), more=TRUE)
print(image(env(fm03)$L, sub=NULL,xlab="L",ylab=NULL,colorkey=FALSE),
      split=c(3,1,3,1))
@   
...
print(image(env(fm04)$Lambda, sub=NULL,xlab=expression(Lambda),ylab=NULL),
      split=c(1,1,3,1), more=TRUE)
print(image(tcrossprod(env(fm04)$Zt),sub=NULL,xlab="Z'Z",ylab=NULL),
      split=c(2,1,3,1), more=TRUE)
print(image(env(fm04)$L, sub=NULL,xlab="L",ylab=NULL,colorkey=FALSE),
      split=c(3,1,3,1))
@   
\caption[Images of $\Lambda$, $\vec Z\trans\vec Z$ and $\vec L$ for
...
memory,
<<fm05Lstats>>=
object.size(env(fm05)$L)
unclass(round(object.size(env(fm05)$L)/2^20, 3))  # size in megabytes
@ 
...
<<nnzero>>=
nnzero(as(env(fm05)$L, "sparseMatrix"))
@
