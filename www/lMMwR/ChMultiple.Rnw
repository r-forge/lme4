...
\begin{figure}[tbp]
  \centering
<<fm2LambdaLimage,fig=TRUE,echo=FALSE,height=3,width=8>>=
...
\subsection[Parameter Estimates for Model \code{fm3}]{Assessing
  Parameter Estimates in Model \texttt{fm3}}
...
that we are comparing the quality of fits obtained with two nested
models.  That is, we are not comparing parameters per se, we are
comparing the model fits obtainable with some constraints on parameter
values to those without the constraints.
...
\end{figure}
for the remaining parameters in model \code{fm3a}
(Fig.~\ref{fig:fm3aprplot}) are similar to the corresponding panels in
Fig.~\ref{fig:fm3prplot}, as confirmed by the numerical values of the
...
\begin{prob}\label{prS1}
  ...
  are a Star Trek fan, you can speculate about what, exactly, the
  ``Borg scale'' is.)  Use
...
\end{prob}
\begin{prob}  %% Give hints on this question.
...
\begin{prob}
  ...
  which allows us to perform comparisons of these specific types.  At
...
\begin{prob}
  ...
  for \code{Type} using
<<plotexam,eval=FALSE>>=
dotplot(ranef(fm, which = "Type", postVar = TRUE), aspect = 0.2, strip = FALSE)
@   
...
\end{prob}
\begin{prob}
  Plot the prediction intervals on each of the three sets of random effects.
\end{prob}
\begin{prob}
