...
ratbrain <- read.delim("http://www-personal.umich.edu/~bwest/rat_brain.dat")
ratbrain$treatment <- factor(ratbrain$treatment,
                             labels = c("Basal", "Carbachol"))
ratbrain$region <- factor(ratbrain$region,
                          labels = c("BST", "LS", "VDB"))
@
...
random-effects term and that sole term was a simple, scalar
random-effects term.  Although such models can be useful, it is with
the facility to use multiple random-effects terms and to use
random-effects terms beyond a simple, scalar term that we can begin to
realize the flexibility and versatility of mixed models.  In this
chapter we consider models with multiple simple, scalar random-effects
terms.  In the next chapter we consider models with random-effects
terms that are more complex than a simple, scalar term.
...
The methods in the \package{lme4} package are able to do this.  Let us
...
has the greatest contribution, then plate-to-plate variability and
finally the ``residual'' variability that cannot be attributed to
either the sample or the plate.  These conclusions are consistent with
what we see in the \code{Penicillin} data plot
(Figure~\ref{fig:Penicillindot}).
...
matrix, $\bm Z$, that is the matrix of indicators of the levels of the
...
concatenated to for the model matrix $\bm Z$.  The transpose of this
matrix contains rows of indicators for each factor, as shown in
Figure~\ref{fig:fm2Ztimage}.
\begin{figure}[tbp]
  \centering
<<fm2Ztimage,fig=TRUE,echo=FALSE,height=2>>=
print(image(fm2@Zt, sub=NULL))
@   
...
The parameter $\bm\theta$ for this model is two-dimensional.
<<fm2verb>>=
getPars(fm2@rCF)
@ 
...
Contours of the profiled REML criterion are shown in
Figure~\ref{fig:fm2profREMLcontours}.
\begin{figure}[tbp]
  \centering
<<fm2profREMLcontours,fig=TRUE,echo=FALSE,height=5>>=
fm2env <- lmer(diameter ~ 1 + (1|plate) + (1|sample),
               Penicillin, doFit = FALSE)
grd <- expand.grid(th1 = seq(0.9,2.9,len = 101),
                   th2 = seq(1.5,15,len = 101))
attr(grd, "out.attrs") <- NULL          # save a bit of space
REML <- apply(t(grd), 2, setPars, x = fm2env)
thetahat <- getPars(fm2@rCF)
print(contourplot(REML ~ th1 + th2, grd,
                  xlab = expression(theta[1]),
                  ylab = expression(theta[2]),                  
                  at = deviance(fm2) +
                  qchisq(c(0.5,0.8,0.9,0.95,0.99), df = 2),
                  labels = list(labels = paste(c(50,80,90,95,99),
                                "%", sep = "")),
                  panel = function(...)
              {
                  panel.grid(h = -1, v = -1)
                  panel.points(thetahat[1], thetahat[2], pch = 3)
                  panel.contourplot(...)
              }
                  ))
@   
  \caption{Contours of the profiled REML criterion for model \code{fm2}.}
  \label{fig:fm2profREMLcontours}
\end{figure}
These contours correspond to joint confidence regions with confidence
levels 50\%, 80\%, 90\%, 95\% and 99\% for these parameters, based on
the REML criterion.
...
nested, she can check if indeed they are nested.
...
Contours of the profiled REML criterion are shown in
Figure~\ref{fig:fm3profREMLcontours}.
\begin{figure}[tbp]
  \centering
<<fm3profREMLcontours,fig=TRUE,echo=FALSE,height=5>>=
fm3env <- lmer(strength ~ 1 + (1|sample) + (1|batch),
               Pastes, doFit = FALSE)
grd <- expand.grid(th1 = seq(1.5,7,len = 101),
                   th2 = seq(0,7,len = 101))
attr(grd, "out.attrs") <- NULL          # save a bit of space
REML <- apply(t(grd[c("th1","th2")]), 2, setPars, x = fm3env)
thetahat <- getPars(fm3@rCF)
print(contourplot(REML ~ th1 + th2, grd,
                  xlab = expression(theta[1]),
                  ylab = expression(theta[2]),                  
                  at = deviance(fm3) +
...
@   
  \caption{Contours of the profiled REML criterion for model \code{fm3}.}
  \label{fig:fm3profREMLcontours}
\end{figure}
...
<<cm3>>=
(cm3 <- lmer(mathgain ~ I(mathkind-450) + sex + minority + ses + housepov + (1|classid) + (1|schoolid), classroom))
@ 

Contours of the profiled REML criterion are shown in
Figure~\ref{fig:cm3profREMLcontours}.
\begin{figure}[tbp]
  \centering
<<cm3profREMLcontours,fig=TRUE,echo=FALSE,height=5>>=
cm3env <- lmer(mathgain ~ I(mathkind-450) + sex + minority + ses + housepov +
               (1|classid) + (1|schoolid),
               classroom, doFit = FALSE)
grd <- expand.grid(th1 = seq(0.09,0.55,len = 101),
                   th2 = seq(0.1,0.55,len = 101))
attr(grd, "out.attrs") <- NULL          # save a bit of space
REML <- apply(t(grd[c("th1","th2")]), 2, setPars, x = cm3env)
thetahat <- getPars(cm3@rCF)
print(contourplot(REML ~ th1 + th2, grd,
                  xlab = expression(theta[1]),
                  ylab = expression(theta[2]),                  
                  at = deviance(cm3) +
...
\section{Fitting linear mixed models with multiple random-effects terms}
\label{sec:multipleRE}


We use more than one random-effects term in the formula when
specifying models with multiple grouping factors for the random
effects.  Interestingly, the model specifications are quite similar
for data with crossed random effects
