...
One of the many famous statements by Albert Einstein is ``Everything
...
models.  That is, we are not assessing parameter values per se; we are
comparing the model fit obtainable with some constraints on parameter
values to that without the constraints.
...
\subsection{Structure of $\vec L$ for model \texttt{fm4}}
\label{sec:fm4L}
...
\begin{prob}\label{pr2.1.1}
  ...
  are a fan of the Star Trek television series and movies, you can
  speculate about what, exactly, the ``Borg scale'' is.)  Use
