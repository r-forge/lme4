...
\chapter{Simple longitudinal data}
...
subject is in the treatment group or the control group, we may wish
...
for multilevel analysis by applying them to a simple longitudinal
example. Because our purpose is to motivate the later presentation of
the statistical and computational techniques we simply provide the
results here and delay until later chapters the detailed
explanations of how the plots are created and how the statistical models
are fit to the data.
...
The data, available as the data frame \code{sleepstudy} in the
\code{"Matrix"} package, consist of the response variable
\code{Reaction}, which is the average of the reaction time
measurements on a given subject for a given day, and two covariates:
\code{Days}, the number of days of sleep deprivation, and
\code{Subject}, the identifier of the subject on which the observation
was made.
...
data.  For data such as these we use the multipanel style of plots
pioneered by Bill Cleveland\index{Cleveland,William} and his coworkers
at Bell Labs as ``Trellis graphics'' and implemented for R in the
\code{"lattice"} package by Deepayan Sarkar\index{Sarkar,Deepayan}.

The most important relationship to plot for longitudinal data on
multiple subjects is the trend of the response over time by subject,
as shown in Figure~\ref{fig:sleepxyplot}.  This plot, in which the
...
pattern Figure~\ref{fig:sleepxyplot} conveys a great deal of
information, the individual time trends for 18 subjects over 10 days
--- a total of 180 points, without being overly cluttered.

In section \ref{sec:longitudinalplots} we discuss the details of
producing this style of plot for longitudinal data and give other
examples of such plots.  Here we concentrate on some of the aspects of
this particular plot; specifically, the use of a reference line, the
choice of the aspect ratio and the ordering of the panels.

A simple linear regression line has been added to each panel of
Figure~\ref{fig:sleepxyplot} to help assess the linearity of the
relationship and to allow comparison of slopes and intercepts.  The
aspect ratio of the panels (ratio of the height to the width) has been
chosen, according to an algorithm described in
...
We can see that for all the subjects except subject 335 reaction time
increases, more-or-less linearly, with days of sleep deprivation.
However, there is considerable variation both in the initial reaction
time and in the daily rate of increase in reaction time.  A question
of interest to the experimenters is whether a subject's rate of
increase is related to the subject's initial reaction time.  To aid in
assessing possible relationships between the per-subject intercept and
slope, the panels in Figure~\ref{fig:sleepxyplot} have been ordered by
increasing intercept of the within-panel regression lines, from left
to right within rows starting on the bottom row.  With such an ordering
of the panels, a systematic relationship between the intercept and the
slope would show up as a systematic trend in slopes across the panels.
There is little evidence in Figure~\ref{fig:sleepxyplot} of such a
systematic relationship between the subject's initial reaction time
and their rate of increase in reaction time with days of sleep
deprivation. 

\begin{figure}[tbp]
  \begin{center}
<<Sl1,fig=TRUE,echo=FALSE,height=3.5>>=
print(plot(confint(lmList(Reaction ~ Days | Subject, sleepstudy),
                   pooled = TRUE), order = 1))
@ 
  \end{center}
  \caption{95\% confidence intervals on the intercept and slope of
    linear regression models fitted to the data for each subject in the
    sleep study.  A pooled estimate of the residual standard deviation
    was used, resulting in a constant width of the intervals across
    subjects for this balanced data set.  The subjects have been
    ordered according to increasing estimated intercept.}
  \label{fig:Sl1}
\end{figure}
Another way of assessing the extent of such a
relationship is to plot confidence intervals on the fitted slopes
and intercepts (Figure~\ref{fig:Sl1}).  In this plot we have again
ordered the subjects according to the estimated intercept.  This
produces an obvious pattern in the intervals for the intercept.  If
there was a systematic relationship between the intercepts and the
slopes we should also see a pattern in the slopes. Although
Figure~\ref{fig:Sl1} does show considerable variation between subjects
in both the intercept and the slope, there is little evidence of a
systematic relationship between the intercept and the slope.

One notable characteristic of Figure~\ref{fig:Sl1} is the consistency
of the widths of the intervals, a result of the data being balanced
and of the use of a common (``pooled'') estimate of the variance of
the random noise term in the regression model.

We say that longitudinal data are \emph{balanced} if each subject is
observed the same number of times if the times of these observations
are consistent across subjects.  In a designed experiment, such as
generated these data, we can reasonably expect the data to be
balanced.  An observational study, on the other hand, will almost
never generate balanced data.  Although the methods that we describe
in this book apply to balanced or unbalanced data, highly unbalanced
data can make interpretation of the results from a fitted model more
difficult, as we shall see in later chapters.
...
\begin{equation}
  \label{eq:yij}
  y_{ij} = \beta_1+\beta_2 x_{ij} + b_{i1} + b_{i2} x_{ij} + \epsilon_{ij},
  \quad i=1,\dots,18, j=1,\dots,10
\end{equation}
where $y_{ij}$ is the $j$th reaction time observed on subject $i$,
taken on day $x_{ij}$, $\bm\beta=(\beta_1,\beta_2)\trans$ is the vector
of fixed effects, $\bm b_i=(b_{i1}, b_{i2})\trans$ is the vector of random
effects for the $i$th subject and $\epsilon_{ij}$ is the unexplained
or ``residual'' random noise associated with this observation.

We assume that the $\epsilon_{ij}$ are independently distributed as
$\epsilon_{ij}\sim\mathcal{N}(0,\sigma^2)$.  In the first form of the
mixed-effects model we assume that the $\bm b_i$ are independently
distributed as $\bm b_i\sim\mathcal{N}(\bm0,\bm\Sigma_1)$ where
$\bm\Sigma_1$ is a general $2\times 2$ variance-covariance matrix.  That
is, in the first model the random effects for the intercept and the
slope of the same subject are allowed to be correlated.  Because our
graphical evaluation of the data in \S\ref{sec:sleep} indicates that
it may be reasonable to model these data with independent random
effects for intercept and slope within subject, we will later fit a
second model that has such independent random effects then compare the
two fitted models.

When fitting mixed-effects models using \code{lmer} we specify the
model to be fit using a formula.  For the first model the formula can
be written
<<formulasm1,echo=FALSE>>=
Reaction ~ 1 + Days + (1 + Days|Subject)
@ 
indicating that the variable \code{Reaction} is modeled by an
intercept and a slope with respect to the variable \code{Days} plus
random effects, grouped by \code{Subject}, for the intercept and the
slope.

That is, the left hand side of the formula, \code{Reaction} in this
case, specifies the response.  Usually the left hand side is the name
of a variable, as it is here, but it can be a more general expression
like \code{log(Response)}.  Expressions on the right hand side of the
formula, \code{1 + Days + (1 + Days|Subject)} in this case, together
specify the fixed effects and the random effects formulation.  A
conditional expression, which is any expression that contains the
vertical bar, \code{|}, designate random effects for the terms on the
left of the \code{|} grouped according to the factor on the right of
the \code{|}.  Expressions on the right hand side of the formula that
do not include the vertical bar specify fixed effects.

The formula as given above explicitly indicates the presence of the
intercept terms.  Howewer, it is common to write a formula like this as
<<formulasm1a,echo=FALSE>>=
Reaction ~ Days + (Days|Subject)
@ 
taking advantage of the fact that the formula language implicitly
includes an intercept term in the fixed effects and in each random
...
We can fit the first model, store the result as \code{sm1}
...
$\widehat{\bm\beta}=(251.41,10.467)\trans$. These represent a typical
...
By fitting model \code{sm2} with independent random effects for
intercept and slope and comparing this fitted model to \code{sm1} we
can assess this claim using a statistical hypothesis test.
<<sm2>>=
(sm2 <- lmer(Reaction ~ Days + (1|Subject) + (0+Days|Subject), sleepstudy))
anova(sm2, sm1)
@ 

We can see that the fitted model \code{sm2} is quite similar to
\code{sm1} except for the obvious difference that there is no
within-subject correlation of the random effects in \code{sm2}.  The
...
\code{sm1} does not fit significantly better than the model \code{sm2}
and hence we prefer the model \code{sm2} which has fewer parameters.
...
pr2 <- profile(sm2)
print(xyplot(pr2, aspect = 1.3, layout = c(5,1), absVal = FALSE))
@ 
  \caption{Signed square root, $\zeta$, of the likelihood ratio test
    statistic for each of the parameters in model \code{fm2}.  The
    vertical lines are the endpoints of 50\%, 80\%, 90\%, 95\% and 99\%
    confidence intervals derived from this test statistic.}
  \label{fig:pr2plt}
\end{figure}
show that confidence intervals $\sigma_1$ and $\sigma_2$ will be slightly
...
the standard normal.  For example, the 95\% confidence intervals from
the profile $\zeta$ are
<<confintpr2>>=
confint(pr2)
@ 
...
print(splom(pr2))
@ 
\caption{Profile pairs plot for the parameters in model \code{fm2}.
...
\subsection{Posterior distributions from model \code{sm2}}
...
parameters in model \code{sm2}.
<<ss2seed,echo=FALSE,results=hide>>=
set.seed(123454321)
@ 
<<ss2>>=
#ss2 <- mcmcsamp(sm2,10000)
@ 
...
%       model \code{Sm2}.  The variance parameters are shown on the
%       logarithm scale.}
...
%     \code{sm2}.  The variance and covariance parameters have been
...
\section{Examining the random effects}
  ...
  \begin{itemize}
  \item Although the random effects $\bm b$ behave like parameters in
  ...
  \item These values are also the modes of the conditional
    distribution (i.e. given the data $\bm y$ and the estimates of
    $\bm\beta$, $\sigma^2$ and $\bm\Sigma$) of $\bm b$.
  \item For linear mixed model the conditional
    distribution $[\bm b|\bm y,\sigma^2,\bm\Sigma]$ is normal (Gaussian)
...
@
  \caption{Scatterplot of the conditional modes, or BLUPs, of the
    random effects for model \code{Sm1}.  Each point represents the
...
### FIXME: Need a coef method
cc1 <- as.data.frame(coef(sm1)$Subject)
names(cc1) <- c("A", "B")
df <- cbind(df, cc1)
with(df,
...
#print(qqmath(ranef(sm1,post=TRUE))[[1]])
@
...
the \code{formula} argument.  For model \code{sm1} the formula is
<<formulasm1,echo=FALSE>>=
Reaction ~ Days + (Days|Subject)
@ 
...
$n\times p$ fixed-effects model matrix $\bm X$ according to the rules
...
fixed-effects parameter vector $\bm\beta$, respectively.  In this case
of model \code{sm1} the only unconditional term, \code{Days}, is the
name of a numeric variable, which is incorporated as a column,
labelled \code{Days} in $\bm X$. By convention, the intercept term,
...
The first few rows of the $180\times 2$ model matrix $\bm X$ for model
\code{sm1} are
<<headmodelmatrixsm1>>=
...
as an $n$-dimensional vector $\bm\epsilon\sim\mathcal{N}(\bm0,\sigma^2\bm I)$
where $\bm I$ is an identity matrix.  That is, the elements of $\bm\epsilon$
...
If we represent all the random effects as a vector $\bm b$, of length
$q=\sum_{i=1}^kq_in_i$, we can write
the model as
\begin{equation}
  \label{eq:lmmGeneral}
  \bm y = \bm X\bm\beta + \bm Z\bm b + \bm\epsilon,\quad
  \bm b\sim\mathcal{N}\left(\bm0,\bm\Sigma\right),
  \bm\epsilon\sim\mathcal{N}\left(\bm0,\sigma^2\bm I\right),\bm b\perp\bm\epsilon
\end{equation}
where $\bm Z$ is an $n\times q$ model matrix generated from the
conditional terms, $\bm\Sigma$ is a $q\times q$ variance-covariance
...
hence the dimensions of $\bm Z$ and $\bm\Sigma$ in the general form
...
For model \code{sm1}, $k=1$, $q_1=2$ and $n_i=18$ so, as we have seen,
...
$\bm\Sigma$ consists of the $2\times 2$ matrix $\bm\Sigma_1$
repeated $18$ times in the pattern
\begin{equation}
  \label{eq:bSigmasm1}
  \bm\Sigma=
  \begin{bmatrix}
    \bm\Sigma_1 & \bm0     & \hdots & \bm0\\
    \bm0     & \bm\Sigma_1 & \hdots & \bm0\\
    \vdots    & \vdots    & \ddots & \vdots\\
    \bm0     & \bm0     & \hdots & \bm\Sigma_1
  \end{bmatrix}
\end{equation}

Also, although the model matrix $\bm Z$ has dimension $180\times 36$,
...
The matrix $\bm Z$ is evaluated and stored as a sparse matrix.  The
first few rows of $\bm Z$ for model \code{sm1} are
<<Zsm1,echo=FALSE>>=
head(t(env(sm1)$Zt))
@ 
...
Easier to understand, perhaps, is the image of $\bm Z\trans$ in
Figure~\ref{fig:sm2Zt}.
\begin{figure}[tbp]
  \centering
<<sm2Zt,fig=TRUE,echo=FALSE,results=hide,height=3,width=8>>=
print(image(env(sm2)$Zt, sub = NULL, xlab = NULL, ylab = NULL))
@
\caption{Image of $\bm Z\trans$, the transpose of $\bm Z$, the random
  effects model matrix in model \code{sm2}}
  \label{fig:sm2Zt}
\end{figure}



In the model $\bm b$ is a random variable
\begin{equation}
  \label{eq:bdist}
  \bm b\sim\mathcal{N}(\bm0,\bm\Sigma)
\end{equation}
and it is the elements of $\bm\Sigma$ that we estimate.  Because we
assume that random effects associated with different grouping factors
are indep
which we can write as
If we write the
model matrix for the entire random effects vector as $\bm Z$ and the
variance-cov
matrix, which we call $\bm Z$, for the random effects vector.

contribute to another model matrix, which we call
$\bm Z$, through a slightly more complicated mechanism.  In a
...
conditional term in the model specification for model \code{sm1}.
...
then the expression contributes $q_in_i$ columns to the matrix $\bm Z$.
