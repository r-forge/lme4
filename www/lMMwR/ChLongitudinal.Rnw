...
models and we begin by doing so.  In section~\ref{sec:fm9re} we will
compare estimates from a mixed-effects model with those from the
within-subject fixed-effects fits.

\section{Mixed-effects Models For the
...
\code{fm8}, allows for correlation (in the unconditional distribution)
...
print(image(fm8@re@Lambda, sub = NULL, xlab = expression(Lambda),
            ylab = NULL),
      split = c(1,1,3,1), more = TRUE)
print(image(tcrossprod(fm8@re@Lambda), sub = NULL,
            xlab = expression(Sigma), ylab = NULL),
      split = c(2,1,3,1), more = TRUE)
print(image(fm8@re@L, sub = NULL, xlab = "L", ylab = NULL),
      split = c(3,1,3,1))
@
...
model (Fig.~\ref{fig:fm8LambdaL}).  The matrix $\Lambda$ has 18
...
high as $10.5 + 2\cdot 6=21.9$ ms./day.  Again, the conclusions from
...
(fm9 <- lmer(Reaction ~ 1 + Days + (1|Subject) + (0+Days|Subject),
             sleepstudy, REML = 0))
@ 
As in model \code{fm8}, there are two random effects for each subject
<<modesfm9>>=
head(ranef(fm9)[["Subject"]])
@ 
but no correlation has been estimated
<<fm9reshow,echo=FALSE>>=
...
\begin{figure}[tbp]
  \centering
<<fm9LambdaL,fig=TRUE,echo=FALSE,results=hide,height=3,width=8>>=
print(image(fm9@re@Lambda, sub = NULL, xlab = expression(Lambda),
            ylab = NULL),
      split = c(1,1,3,1), more = TRUE)
print(image(tcrossprod(fm9@re@Lambda), sub = NULL,
            xlab = expression(Sigma), ylab = NULL),
      split = c(2,1,3,1), more = TRUE)
print(image(fm9@re@L, sub = NULL, xlab = "L", ylab = NULL),
      split = c(3,1,3,1))
@
\caption[Images of $\Lambda$, $\Sigma$ and $\vec L$ for model
\code{fm9}]{Images of $\Lambda$, the relative covariance factor,
  $\Sigma$, the variance-covariance matrix of the random effects, and
  $\vec L$, the sparse Cholesky factor, in model \code{fm9}}
  \label{fig:fm9LambdaL}
\end{figure}

Images of the matrices $\Lambda$, $\Sigma$ and $\vec L$
(Fig.~\ref{fig:fm9LambdaL}) show that $\Sigma$ is indeed diagonal.
The order of the random effects in $\Sigma$ and $\Lambda$ for model
\code{fm9} is different from the order in model \code{fm8}.  In model
\code{fm8} the two random effects for a particular subject were
adjacent.  In model \code{fm9} all the intercept random effects occur
...
Sect.~\ref{sec:fill-reducingP}) calculated for model \code{fm9}
provides a post-ordering to group random effects with similar
structure in $\vec Z$.
\begin{figure}[tbp]
  \centering
<<fm89Zt,fig=TRUE,echo=FALSE,results=hide,height=4,width=8>>=
print(image(fm8@re@Zt, sub = NULL, xlab = NULL, ylab = NULL,
            scales = list(x = list(labels = NULL))),
      pos = c(0,0.38,1,1), more = TRUE)
print(image(fm9@re@Zt, sub = NULL, xlab = NULL, ylab = NULL,
            scales = list(x = list(labels = NULL))),
      pos = c(0,0,1,0.62))
@
\caption[Images of $\vec Z\trans$ for models \code{fm8} and
\code{fm9}]{Images of $\vec Z\trans$ for models \code{fm8} (upper
  panel) and \code{fm9} (lower panel)}
\label{fig:fm89Zt}
\end{figure}

Images of $\vec Z\trans$ for these two models (Fig.~\ref{fig:fm89Zt})
...
of $\vec Z$ for model \code{fm9}.  The first 18 columns (rows in the
bottom panel of Fig.~\ref{fig:fm89Zt}) are the indicator columns for
...
the non-zeros are determined by the \code{Days} factor.  In other
words, these columns are formed by the \emph{interaction} of the
numeric covariate, \code{Days}, and the categorical covariate,
\code{Subject}.

The non-zero values in the model matrix $\vec Z$ for model \code{fm8}
are the same as those for model \code{fm8} but the columns are in a
...
columns that are considered as 18 adjacent pairs.  The values within
...
$i$th \code{expr} with the $k$ indicator columns for the factor
\code{fac}.  The diagonal block in $\Lambda$ is itself block diagonal,
consisting of $k_i$ blocks, each a lower triangular matrix of size
$s_i$.  In fact, these inner blocks are repetitions of the same lower
triangular $s_i\times s_i$ matrix.  The $i$ term contributes
$s_i(s_i+1)/2$ elements to the variance-component parameter,
$\vec\theta$, and these are the elements in the lower triangle of this
$s_i\times s_i$ template matrix.
...
the formula for model \code{fm8} as
...
the intercept explicitly in the formula so as to emphasize the
relationship between terms in the formula and coefficients or random
effects in the model.  Others omit these implicit terms so as to
economize on the amount of typing required.  Either approach can be
used.  The important point to remember is that the intercept must be
...
terms.  That is, we could write the formula for model \code{fm1} in
...
\subsection{Comparing Models \code{fm9} and \code{fm8}}
\label{sec:comparingfm8fm9}

Returning to models \code{fm8} and \code{fm9} for the
\code{sleepstudy} data, it is easy to see that these are nested models
because \code{fm8} is reduced to \code{fm9} by constraining the
...
Fig.~\ref{fig:fm8LambdaL} to be zero).
...
parameter in model \code{fm8} relative to \code{fm9} does not produce
a significantly better fit.  By the principal of parsimony we prefer
the reduced model, \code{fm9}.
...
The profile pairs plot (Fig.~\ref{fig:pr9pairs})
\begin{figure}[tbp]
  \centering
<<pr9pairs,fig=TRUE,echo=FALSE,height=8>>=
print(splom(pr9))
@
\caption[Profile pairs plot for the parameters in model
\code{fm9}]{Profile pairs plot for the parameters in model \code{fm9}.
...
themselves.  Model \code{fm9} is the first model we have fit with more
...
\begin{figure}[tbp]
  \centering
<<ranefcoeffm9,fig=TRUE,echo=FALSE,height=4>>=
print(plot(rr1, type = c("g","p"), aspect = 1)$Subject,
      split = c(1,1,2,1), more = TRUE)
print(plot(coef(fm9), type = c("g","p"), aspect = 1)$Subject,
      split = c(2,1,2,1))
@ 
  \caption{Plot of the conditional modes of the random effects for model \code{fm9} (left panel) and the corresponding subject-specific coefficients (right panel)}
  \label{fig:ranefcoeffm9}
\end{figure}
...
is a scatterplot.  For three or more random effects per level level of
the grouping factor, the plot is a scatterplot matrix.  The left hand panel in Fig.~\ref{fig:ranefcoeffm9} was created with \code{plot(ranef(fm9))}.
...
the names of coefficients.  For model \code{fm9} the fixed-effects
...
as in the right hand panel of Fig.~\ref{fig:ranefcoeffm9}.  By comparing
the two panels in Fig.~\ref{fig:ranefcoeffm9} we can see that the
...
for \code{fm9}]{Comparison of the within-subject estimates of the
  ...
  an arrow are the within-subject and conditional mode estimates for
  the same subject.  The arrow points from the within-subject estimate
  to the conditional mode for the mixed-effects model.  The subject
...
and intercept calculated from model \code{fm9}.  We see that, in
...
subjects ``borrowing strength'' from each other.  This is a
...
\code{fm9}]{Comparison of the predictions from the within-subject fits
  with those from the conditional modes of the subject-specific
  parameters in the mixed-effects model.}
  \label{fig:shrinkfit}
\end{figure}

The predictions from model \code{fm9} and from the within-subject
least squares fits for each subject are shown in
Fig.~\ref{fig:shrinkfit}.  In may seem that the shrinkage from the
...
fidelity in Sec.~\ref{sec:IntegratingH}, where we make the
mathematical nature of this balance explicit.

We should also examine
\begin{figure}[tbp]
%% FIXME: ranef(fm9, postVar = TRUE) currently throws an error.  
  \centering
<<caterpillar,fig=TRUE,echo=FALSE,results=hide,height=4>>=
print(dotplot(ranef(fm8,post=TRUE),
              scales = list(x = list(relation = "free")))[[1]])
@
  \caption{Prediction intervals on the random effects for model \code{fm9}.}
...
model \code{fm9}, which does not allow for correlation of the random
