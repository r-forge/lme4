...
\SweaveOpts{prefix=TRUE,prefix.string=figs/Long,include=FALSE}
...
@ 
\begin{figure}[tbp]
  \centerline{\includegraphics[width=\textwidth]{figs/Long-sleepxyplot}}
...
<<Sl1,fig=TRUE,echo=FALSE,height=3.2,width=7>>=
#print(plot(confint(lmList(Reaction ~ Days | Subject, sleepstudy),
### FIXME: Create the necessary confint plot.
#                   pooled = TRUE), order = 1))
@ 
\begin{figure}[tbp]
  \centerline{\includegraphics[width=\textwidth]{figs/Long-Sl1}}
...
taken on day $x_{ij}$, $\bbeta=(\beta_1,\beta_2)\trans$ is the vector
of fixed effects, $\bb_i=(b_{i1}, b_{i2})\trans$ is the vector of random
...
mixed-effects model we assume that the $\bb_i$ are independently
distributed as $\bb_i\sim\mathcal{N}(\bzer,\bSigma_1)$ where
$\bSigma_1$ is a general $2\times 2$ variance-covariance matrix.  That
...
$\widehat{\bbeta}=(251.41,10.467)\trans$. These represent a typical
...
\section{Examining the random effects}
  ...
  \begin{itemize}
  \item Although the random effects $\bb$ behave like parameters in
  ...
  \item These values are also the modes of the conditional
    distribution (i.e. given the data $\by$ and the estimates of
    $\bbeta$, $\sigma^2$ and $\bSigma$) of $\bb$.
  \item For linear mixed model the conditional
    distribution $[\bb|\by,\sigma^2,\bSigma]$ is normal (Gaussian)
...
@
\pgfdeclareimage[width=\textwidth]{rr1plot}{figs/Long-rr1plot}

\begin{figure}[tbp]
  \centerline{\pgfuseimage{rr1plot}}
...
@ %$
\pgfdeclareimage[width=\textwidth]{shrinkage}{figs/Long-shrinkage}

\begin{figure}[tbp]
  \centerline{\pgfuseimage{shrinkage}}
...
@ 
\pgfdeclareimage[width=\textwidth]{shrinkfit}{figs/Long-shrinkfit}

\begin{figure}[tbp]
  \centerline{\pgfuseimage{shrinkfit}}
...
print(qqmath(ranef(sm1,post=TRUE))$Subject)
@ %$

\pgfdeclareimage[width=\textwidth]{caterpillar}{figs/Long-caterpillar}

\begin{figure}[tbp]
  \centerline{\pgfuseimage{caterpillar}}
...
$n\times p$ fixed-effects model matrix $\bX$ according to the rules
...
fixed-effects parameter vector $\bbeta$, respectively.  In this case
...
labelled \code{Days} in $\bX$. By convention, the intercept term,
...
The first few rows of the $180\times 2$ model matrix $\bX$ for model
\code{sm1} are
<<headmodelmatrixsm1,eval=FALSE>>=
#FIXME: Write model.matrix and model.response methods for the lmer class
...
as an $n$-dimensional vector $\beps\sim\mathcal{N}(\bzer,\sigma^2\bI)$
where $\bI$ is an identity matrix.  That is, the elements of $\beps$
...
If we represent all the random effects as a vector $\bb$, of length
$q=\sum_{i=1}^kq_in_i$, we can write
the model as
\begin{equation}
  \label{eq:lmmGeneral}
  \by = \bX\bbeta + \bZ\bb + \beps,\quad
  \bb\sim\mathcal{N}\left(\bzer,\bSigma\right),
  \beps\sim\mathcal{N}\left(\bzer,\sigma^2\bI\right),\bb\perp\beps
\end{equation}
where $\bZ$ is an $n\times q$ model matrix generated from the
conditional terms, $\bSigma$ is a $q\times q$ variance-covariance
...
hence the dimensions of $\bZ$ and $\bSigma$ in the general form
...
$\bSigma$ consists of the $2\times 2$ matrix $\bSigma_1$
repeated $18$ times in the pattern
\begin{equation}
  \label{eq:bSigmasm1}
  \bSigma=
  \begin{bmatrix}
    \bSigma_1 & \bzer     & \hdots & \bzer\\
    \bzer     & \bSigma_1 & \hdots & \bzer\\
    \vdots    & \vdots    & \ddots & \vdots\\
    \bzer     & \bzer     & \hdots & \bSigma_1
  \end{bmatrix}
\end{equation}

Also, although the model matrix $\bZ$ has dimension $180\times 36$,
...
The matrix $\bZ$ is evaluated and stored as a sparse matrix.  The
first few rows of $\bZ$ for model \code{sm1} are
<<Zsm1,echo=FALSE>>=
head(t(sm1@Zt))
@ 
...
In the model $\bb$ is a random variable
\begin{equation}
  \label{eq:bdist}
  \bb\sim\mathcal{N}(\bzer,\bSigma)
\end{equation}
and it is the elements of $\bSigma$ that we estimate.  Because we
assume that random effects associated with different grouping factors
are indep
which we can write as
If we write the
model matrix for the entire random effects vector as $\bZ$ and the
variance-cov
matrix, which we call $\bZ$, for the random effects vector.

contribute to another model matrix, which we call
$\bZ$, through a slightly more complicated mechanism.  In a
...
then the expression contributes $q_in_i$ columns to the matrix $\bZ$.
