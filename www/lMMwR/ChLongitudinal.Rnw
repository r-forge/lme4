...
library(minqa)
library(lme4a)
fm07 <- lmer(Reaction ~ Days + (1|Subject) + (0+Days|Subject),
...
models and we begin by doing so.  In section~\ref{sec:fm07re} we will
...
\code{fm06}, allows for correlation (in the unconditional distribution)
...
<<fm06reshow,echo=FALSE>>=
cat(paste(capture.output(print(fm06))[8:11], collapse = "\n"), "\n")
@
...
\begin{figure}[tbp]
  \centering
<<fm06LambdaL,fig=TRUE,echo=FALSE,results=hide,height=3,width=8>>=
print(image(fm06@re@Lambda, sub = NULL, xlab = expression(Lambda),
            ylab = NULL),
      split = c(1,1,3,1), more = TRUE)
print(image(tcrossprod(fm06@re@Lambda), sub = NULL,
            xlab = expression(Sigma), ylab = NULL),
      split = c(2,1,3,1), more = TRUE)
print(image(fm06@re@L, sub = NULL, xlab = "L", ylab = NULL),
      split = c(3,1,3,1))
@
  \caption{Images of $\Lambda$, $\Sigma$ and $\vec L$ for model \code{fm06}}
  \label{fig:fm06LambdaL}
\end{figure}
...
model (Fig.~\ref{fig:fm06LambdaL}).  The matrix $\Lambda$ has 18
...
(fm07 <- lmer(Reaction ~ 1 + Days + (1|Subject) + (0+Days|Subject),
             sleepstudy, REML=FALSE))
@ 
As in model \code{fm06}, there are two random effects for each subject
<<modesfm07>>=
head(ranef(fm07)[["Subject"]])
@ 
but no correlation has been estimated
<<fm07reshow,echo=FALSE>>=
cat(paste(capture.output(print(fm07))[8:11], collapse = "\n"), "\n")
@
...
\begin{figure}[tbp]
  \centering
<<fm07LambdaL,fig=TRUE,echo=FALSE,results=hide,height=3,width=8>>=
print(image(fm07@re@Lambda, sub = NULL, xlab = expression(Lambda),
            ylab = NULL),
      split = c(1,1,3,1), more = TRUE)
print(image(tcrossprod(fm07@re@Lambda), sub = NULL,
            xlab = expression(Sigma), ylab = NULL),
      split = c(2,1,3,1), more = TRUE)
print(image(fm07@re@L, sub = NULL, xlab = "L", ylab = NULL),
      split = c(3,1,3,1))
@
\caption[Images of $\Lambda$, $\Sigma$ and $\vec L$ for model
\code{fm07}]{Images of $\Lambda$, the relative covariance factor,
  $\Sigma$, the variance-covariance matrix of the random effects, and
  $\vec L$, the sparse Cholesky factor, in model \code{fm07}}
  \label{fig:fm07LambdaL}
\end{figure}

Images of the matrices $\Lambda$, $\Sigma$ and $\vec L$
(Fig.~\ref{fig:fm07LambdaL}) show that $\Sigma$ is indeed diagonal.
The order of the random effects in $\Sigma$ and $\Lambda$ for model
\code{fm07} is different from the order in model \code{fm06}.  In model
\code{fm06} the two random effects for a particular subject were
adjacent.  In model \code{fm07} all the intercept random effects occur
...
Sect.~\ref{sec:fill-reducingP}) calculated for model \code{fm07}
provides a post-ordering to group random effects with similar
structure in $\vec Z$.
\begin{figure}[tbp]
  \centering
<<fm069Zt,fig=TRUE,echo=FALSE,results=hide,height=4,width=8>>=
print(image(fm06@re@Zt, sub = NULL, xlab = NULL, ylab = NULL,
            scales = list(x = list(labels = NULL))),
      pos = c(0,0.38,1,1), more = TRUE)
print(image(fm07@re@Zt, sub = NULL, xlab = NULL, ylab = NULL,
            scales = list(x = list(labels = NULL))),
      pos = c(0,0,1,0.62))
@
\caption[Images of $\vec Z\trans$ for models \code{fm06} and
\code{fm07}]{Images of $\vec Z\trans$ for models \code{fm06} (upper
  panel) and \code{fm07} (lower panel)}
\label{fig:fm069Zt}
\end{figure}

Images of $\vec Z\trans$ for these two models (Fig.~\ref{fig:fm069Zt})
...
of $\vec Z$ for model \code{fm07}.  The first 18 columns (rows in the
bottom panel of Fig.~\ref{fig:fm069Zt}) are the indicator columns for
...
The non-zero values in the model matrix $\vec Z$ for model \code{fm06}
are the same as those for model \code{fm06} but the columns are in a
...
the formula for model \code{fm06} as
<<formulafm06,echo=FALSE>>=
...
\subsection{Comparing Models \texttt{fm07} and \texttt{fm06}}
\label{sec:comparingfm06fm07}

Returning to models \code{fm06} and \code{fm07} for the
\code{sleepstudy} data, it is easy to see that these are nested models
because \code{fm06} is reduced to \code{fm07} by constraining the
...
Fig.~\ref{fig:fm06LambdaL} to be zero).
...
parameter in model \code{fm06} relative to \code{fm07} does not produce
a significantly better fit.  By the principal of parsimony we prefer
the reduced model, \code{fm07}.
...
\begin{figure}[tbp]
  \centering
<<pr07plt,fig=TRUE,echo=FALSE,height=3.5>>=
print(xyplot(pr07, aspect = 1.3, layout = c(5,1)))
@
\caption[Profile zeta plots for the parameters in model
\code{fm07}]{Profile zeta plot for each of the parameters in model
  \code{fm07}.  The vertical lines are the endpoints of 50\%, 80\%,
  90\%, 95\% and 99\% profile-based confidence intervals for each
  parameter.}
  \label{fig:pr07plt}
\end{figure}
for the parameters in model \code{fm07} (Fig.~\ref{fig:pr07plt}) show
...
The profile pairs plot (Fig.~\ref{fig:pr07pairs})
\begin{figure}[tbp]
  \centering
<<pr07pairs,fig=TRUE,echo=FALSE,height=8>>=
print(splom(pr07))
@
\caption[Profile pairs plot for the parameters in model
\code{fm07}]{Profile pairs plot for the parameters in model \code{fm07}.
...
themselves.  Model \code{fm07} is the first model we have fit with more
...
\begin{figure}[tbp]
  \centering
<<ranefcoeffm07,fig=TRUE,echo=FALSE,height=4>>=
print(plot(rr1, type = c("g","p"), aspect = 1)$Subject,
      split = c(1,1,2,1), more = TRUE)
print(plot(coef(fm07), type = c("g","p"), aspect = 1)$Subject,
      split = c(2,1,2,1))
@ 
  \caption{Plot of the conditional modes of the random effects for model \code{fm07} (left panel) and the corresponding subject-specific coefficients (right panel)}
  \label{fig:ranefcoeffm07}
\end{figure}
...
panel in Fig.~\ref{fig:ranefcoeffm07} was created with
\code{plot(ranef(fm07))}.
...
the names of coefficients.  For model \code{fm07} the fixed-effects
...
as in the right hand panel of Fig.~\ref{fig:ranefcoeffm07}.  By comparing
the two panels in Fig.~\ref{fig:ranefcoeffm07} we can see that the
...
cc1 <- as.data.frame(coef(fm07)$Subject)
names(cc1) <- c("A", "B")
df <- cbind(df, cc1)
ff <- fixef(fm07)
with(df,
...
for \code{fm07}]{Comparison of the within-subject estimates of the
...
and intercept calculated from model \code{fm07}.  We see that, in
...
\code{fm07}]{Comparison of the predictions from the within-subject fits
  with those from the conditional modes of the subject-specific
  parameters in the mixed-effects model.}
  \label{fig:shrinkfit}
\end{figure}

The predictions from model \code{fm07} and from the within-subject
...
\begin{figure}[tbp]
%% FIXME: ranef(fm07, postVar = TRUE) currently throws an error.  
  \centering
<<caterpillar,fig=TRUE,echo=FALSE,results=hide,height=4>>=
print(dotplot(ranef(fm06,post=TRUE),
              scales = list(x = list(relation = "free")))[[1]])
@
  \caption{Prediction intervals on the random effects for model \code{fm07}.}
...
model \code{fm07}, which does not allow for correlation of the random
