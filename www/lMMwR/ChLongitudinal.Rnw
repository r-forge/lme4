...
library(minqa)
library(lme4a)
fm09 <- lmer(Reaction ~ Days + (1|Subject) + (0+Days|Subject),
            sleepstudy, REML=FALSE)
if (file.exists("pr09.rda")) {
    load("pr09.rda")
} else {
    pr09 <- profile(fm09)
    save(pr09, file = "pr09.rda")
}
...
models and we begin by doing so.  In section~\ref{sec:fm09re} we will
...
\code{fm08}, allows for correlation (in the unconditional distribution)
...
<<fm08reshow,echo=FALSE>>=
cat(paste(capture.output(print(fm08))[8:11], collapse = "\n"), "\n")
@
...
\begin{figure}[tbp]
  \centering
<<fm08LambdaL,fig=TRUE,echo=FALSE,results=hide,height=3,width=8>>=
print(image(fm08@re@Lambda, sub = NULL, xlab = expression(Lambda),
            ylab = NULL),
      split = c(1,1,3,1), more = TRUE)
print(image(tcrossprod(fm08@re@Lambda), sub = NULL,
            xlab = expression(Sigma), ylab = NULL),
      split = c(2,1,3,1), more = TRUE)
print(image(fm08@re@L, sub = NULL, xlab = "L", ylab = NULL),
      split = c(3,1,3,1))
@
  \caption{Images of $\Lambda$, $\Sigma$ and $\vec L$ for model \code{fm08}}
  \label{fig:fm08LambdaL}
\end{figure}
...
model (Fig.~\ref{fig:fm08LambdaL}).  The matrix $\Lambda$ has 18
...
<<fm09>>=
(fm09 <- lmer(Reaction ~ 1 + Days + (1|Subject) + (0+Days|Subject),
             sleepstudy, REML=FALSE))
@ 
As in model \code{fm08}, there are two random effects for each subject
<<modesfm09>>=
head(ranef(fm09)[["Subject"]])
@ 
but no correlation has been estimated
<<fm09reshow,echo=FALSE>>=
cat(paste(capture.output(print(fm09))[8:11], collapse = "\n"), "\n")
@
...
\begin{figure}[tbp]
  \centering
<<fm09LambdaL,fig=TRUE,echo=FALSE,results=hide,height=3,width=8>>=
print(image(fm09@re@Lambda, sub = NULL, xlab = expression(Lambda),
            ylab = NULL),
      split = c(1,1,3,1), more = TRUE)
print(image(tcrossprod(fm09@re@Lambda), sub = NULL,
            xlab = expression(Sigma), ylab = NULL),
      split = c(2,1,3,1), more = TRUE)
print(image(fm09@re@L, sub = NULL, xlab = "L", ylab = NULL),
      split = c(3,1,3,1))
@
\caption[Images of $\Lambda$, $\Sigma$ and $\vec L$ for model
\code{fm09}]{Images of $\Lambda$, the relative covariance factor,
  $\Sigma$, the variance-covariance matrix of the random effects, and
  $\vec L$, the sparse Cholesky factor, in model \code{fm09}}
  \label{fig:fm09LambdaL}
\end{figure}

Images of the matrices $\Lambda$, $\Sigma$ and $\vec L$
(Fig.~\ref{fig:fm09LambdaL}) show that $\Sigma$ is indeed diagonal.
The order of the random effects in $\Sigma$ and $\Lambda$ for model
\code{fm09} is different from the order in model \code{fm08}.  In model
\code{fm08} the two random effects for a particular subject were
adjacent.  In model \code{fm09} all the intercept random effects occur
...
Sect.~\ref{sec:fill-reducingP}) calculated for model \code{fm09}
provides a post-ordering to group random effects with similar
structure in $\vec Z$.
\begin{figure}[tbp]
  \centering
<<fm089Zt,fig=TRUE,echo=FALSE,results=hide,height=4,width=8>>=
print(image(fm08@re@Zt, sub = NULL, xlab = NULL, ylab = NULL,
            scales = list(x = list(labels = NULL))),
      pos = c(0,0.38,1,1), more = TRUE)
print(image(fm09@re@Zt, sub = NULL, xlab = NULL, ylab = NULL,
            scales = list(x = list(labels = NULL))),
      pos = c(0,0,1,0.62))
@
\caption[Images of $\vec Z\trans$ for models \code{fm08} and
\code{fm09}]{Images of $\vec Z\trans$ for models \code{fm08} (upper
  panel) and \code{fm09} (lower panel)}
\label{fig:fm089Zt}
\end{figure}

Images of $\vec Z\trans$ for these two models (Fig.~\ref{fig:fm089Zt})
...
of $\vec Z$ for model \code{fm09}.  The first 18 columns (rows in the
bottom panel of Fig.~\ref{fig:fm089Zt}) are the indicator columns for
...
The non-zero values in the model matrix $\vec Z$ for model \code{fm08}
are the same as those for model \code{fm08} but the columns are in a
...
the same lower triangular $s_i\times s_i$ matrix.  The $i$ term
...
the formula for model \code{fm08} as
<<formulafm08,echo=FALSE>>=
...
\subsection{Comparing Models \texttt{fm09} and \texttt{fm08}}
\label{sec:comparingfm08fm09}

Returning to models \code{fm08} and \code{fm09} for the
\code{sleepstudy} data, it is easy to see that these are nested models
because \code{fm08} is reduced to \code{fm09} by constraining the
...
Fig.~\ref{fig:fm08LambdaL} to be zero).
...
parameter in model \code{fm08} relative to \code{fm09} does not produce
a significantly better fit.  By the principal of parsimony we prefer
the reduced model, \code{fm09}.
...
\begin{figure}[tbp]
  \centering
<<pr09plt,fig=TRUE,echo=FALSE,height=3.5>>=
print(xyplot(pr09, aspect = 1.3, layout = c(5,1)))
@
\caption[Profile zeta plots for the parameters in model
\code{fm09}]{Profile zeta plot for each of the parameters in model
  \code{fm09}.  The vertical lines are the endpoints of 50\%, 80\%,
  90\%, 95\% and 99\% profile-based confidence intervals for each
  parameter.}
  \label{fig:pr09plt}
\end{figure}
for the parameters in model \code{fm09} (Fig.~\ref{fig:pr09plt}) show
...
The profile pairs plot (Fig.~\ref{fig:pr09pairs})
\begin{figure}[tbp]
  \centering
<<pr09pairs,fig=TRUE,echo=FALSE,height=8>>=
print(splom(pr09))
@
\caption[Profile pairs plot for the parameters in model
\code{fm09}]{Profile pairs plot for the parameters in model \code{fm09}.
...
themselves.  Model \code{fm09} is the first model we have fit with more
...
\begin{figure}[tbp]
  \centering
<<ranefcoeffm09,fig=TRUE,echo=FALSE,height=4>>=
print(plot(rr1, type = c("g","p"), aspect = 1)$Subject,
      split = c(1,1,2,1), more = TRUE)
print(plot(coef(fm09), type = c("g","p"), aspect = 1)$Subject,
      split = c(2,1,2,1))
@ 
  \caption{Plot of the conditional modes of the random effects for model \code{fm09} (left panel) and the corresponding subject-specific coefficients (right panel)}
  \label{fig:ranefcoeffm09}
\end{figure}
...
panel in Fig.~\ref{fig:ranefcoeffm09} was created with
\code{plot(ranef(fm09))}.
...
the names of coefficients.  For model \code{fm09} the fixed-effects
...
as in the right hand panel of Fig.~\ref{fig:ranefcoeffm09}.  By comparing
the two panels in Fig.~\ref{fig:ranefcoeffm09} we can see that the
...
cc1 <- as.data.frame(coef(fm09)$Subject)
names(cc1) <- c("A", "B")
df <- cbind(df, cc1)
ff <- fixef(fm09)
with(df,
...
for \code{fm09}]{Comparison of the within-subject estimates of the
  ...
  the random for a particular subject.  The arrow points from the
  within-subject estimate to the conditional mode for the
  mixed-effects model.  The subject identifier number is at the head
  of each arrow.}
  \label{fig:shrinkage}
\end{figure}
...
and intercept calculated from model \code{fm09}.  We see that, in
...
\code{fm09}]{Comparison of the predictions from the within-subject fits
  with those from the conditional modes of the subject-specific
  parameters in the mixed-effects model.}
  \label{fig:shrinkfit}
\end{figure}

The predictions from model \code{fm09} and from the within-subject
...
\begin{figure}[tbp]
%% FIXME: ranef(fm09, postVar = TRUE) currently throws an error.  
  \centering
<<caterpillar,fig=TRUE,echo=FALSE,results=hide,height=4>>=
print(dotplot(ranef(fm08,post=TRUE),
              scales = list(x = list(relation = "free")))[[1]])
@
  \caption{Prediction intervals on the random effects for model \code{fm09}.}
...
model \code{fm09}, which does not allow for correlation of the random
