...
taken on day $x_{ij}$, $\vec\beta=(\beta_1,\beta_2)\trans$ is the vector
of fixed effects, $\vec b_i=(b_{i1}, b_{i2})\trans$ is the vector of random
...
mixed-effects model we assume that the $\vec b_i$ are independently
distributed as $\vec b_i\sim\mathcal{N}(\vec0,\Sigma_1)$ where
$\Sigma_1$ is a general $2\times 2$ variance-covariance matrix.  That
...
first few rows of $\vec Z$ for model \code{sm1} are
<<Zsm1,echo=FALSE>>=
head(t(env(sm1)$Zt))
@ 
