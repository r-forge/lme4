...
and a slope (w.r.t. \code{Days}) in the fixed effects and in the
random effects.  In linear model formulas the intercept term is
implicit.  If we had written the formula as
...
The images of $\Lambda$, $\Sigma$ and $\vec L$ for this model
...
the interpretation of the model formulae until Sect.~\ref{sec:model_form}.
...
Sect~\ref{sec:definitions}), relative to the change in the number of
...
Plots of the profile $\zeta$ (Figure~\ref{fig:pr9plt})
\begin{figure}[ht]
  \centering
...
The profile pairs plot (Figure~\ref{fig:pr9pairs})
\begin{figure}[ht]
  \centering
...
@
\caption[Conditional modes of the random effects for model
\code{fm8}]{Scatterplot of the conditional modes, or BLUPs, of the
  random effects for model \code{fm8}.  Each point represents the mode
  of the distribution of the random effects for the intercept and
  slope associated with one of the subjects.}
  \label{fig:rr1plot}
\end{figure}
...
for \code{fm8}]{Comparison of the within-subject estimates of the
  ...
  to the conditional mode for the mixed-effects model.}
  \label{fig:shrinkage}
\end{figure}
\begin{figure}[tbp]
  \centering
...
\code{fm8}]{Comparison of the predictions from the within-subject fits
