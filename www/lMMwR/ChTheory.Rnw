...
In this chapter we describe some of the details the computational
methods for fitting mixed-effects models, as implemented in the
...
predictor, $\vec X\vec\beta+\vec Z\vec b$ is a nonlinear relationship.
...
vector, $\bc Y$.  The unconditional distribution of $\bc B$ and the
conditional distribution of $\bc Y$, given $\bc B=\vec b$, are
...
$\Sigma_\theta$ approaches singularity, as it can.  It is a fact that
singular (i.e. non-invertible) $\Sigma_\theta$ can and do occur in
...
$\vec\theta$ that produce a singular $\Sigma_\theta$.  Because of this
we will take care to use computational methods that can be applied
even when $\Sigma_\theta$ is singular and are stable as
$\Sigma_\theta$ approaches singularity.

A relative covariance factor,
...
The numerical values of the probability density and the likelihood are
the same but the interpretation is different.  For the density we
think of the parameters as being fixed and the value of $\vec y$ as
the variable.  For the likelihood we think of $\vec y$ as fixed at the
observed vector of responses and the parameters, $\vec\theta$,
...
that maximizes the density.  The value $\tilde{\vec u}$ is called the
conditional mode of $\vec u$, given the observed $\vec y$, because it
maximizes the conditional density of $\bc U$ given $\bc Y=\vec y$.
The location of the maximum can also be determined by maximizing the
...
It is easiest to consider the product of these densities on the
deviance scale (negative twice the logarithm of the density).
\begin{equation}
  \label{eq:devh}
...
\end{equation}
Because of the negative sign, $\tilde{\vec u}$ will be the value of
$\vec u$ that minimizes the expression on the right of (\ref{eq:devh}).
...
the minimizer, $\tilde{\vec u}$ is called the \emph{penalized least
  squares} (PLS) solution.  They are given these names because the first term in
the objective, $\left\| \vec y-\vec X\vec\beta-\vec
...
to the observed data, producing a small residual sum of squares,
against simplicity of the model, small values of $\|\vec u\|$.  In
this sense it is the type of criterion called a smoothing objective.
...
\end{equation}
Notice that $\vec\beta$ is only involved in the linear predictor
expression in (\ref{eq:r2thetabeta}).  We will see that $\tilde{\vec
  u}$ can be determined by a direct (i.e. non-iterative) calculation
and, in fact, we can minimize the PRSS criterion with respect to $\vec
u$ and $\vec\beta$ simultaneously without iterating.  We write this
minimum value as
\begin{equation}
  \label{eq:r2theta}
...
\subsection{Determining the PLS Solutions, $\tilde{\vec u}$ and
  $\widehat{\vec\beta}_\theta$}
\label{sec:PLSsol}
...
In earlier chapters we have seen the often the random effects vector
...
columns of the model matrix, $\vec Z\vec\Lambda_\theta$, does not
affect the theory of linear mixed models but can have a profound
effect on the time and storage required to evaluate $\vec L_\theta$ in
large problems.  We write the effect of the permutation as
multiplication by a $q\times q$ \emph{permutation matrix}, $\vec P$,
although in practice we perform the permutation without ever
constructing $\vec P$.  The matrix is for notation only.  It is
determined by the structure of $\Lambda_\theta\trans\vec Z\trans\vec
Z\Lambda_\theta+\vec I_q$ for the initial value of $\vec\theta$ in the
optimization of the profiled deviance.  The permutation itself does
not depend on $\vec\theta$ as long as $\vec\theta$ is not on the
boundary of the parameter space, where it generates a singular
$\Lambda_\theta$.

Thus we alter the definition of the sparse Cholesky factor to be the
sparse lower triangular $q\times q$ matrix that satisfies
\begin{equation}
...
$\vec L_\theta$.  The factor is non-zero at every non-zero of the
lower triangle of the matrix being decomposed.  However, as we saw in
Fig.~\ref{fig:fm2LambdaLimage} of Sect.~\ref{sec:PenicillinModel},
there may be positions in the factor that get filled-in even though
they are known to be zero in the matrix being decomposed.  The
\emph{fill-reducing permutation} is chosen according to certain
heuristics to reduce the amount of fill-in.  We use the approximate
minimal degree (AMD) method described in \citet{Davis:1996}. After the
fill-reducing permutation is determined, a ``post-ordering'' is
applied.  This has the effect of concentrating the non-zeros near the
diagonal of the factor. See \citet{davis06:csparse_book} for more
details.
...
and apply a change of variable to
\begin{equation}
  \label{eq:changeVar}
...
logarithm of the likelihood, becomes
\begin{equation}
...
Evaluating the determinant, $|\vec L_\theta|$, may seem formidable
but, because $\vec L$ is triangular, its determinant is simply the
...
\section{Generalizations to Other Mixed Models}
\label{sec:general}
