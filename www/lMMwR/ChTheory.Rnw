...
marginal distribution of $\bc Y$, which in this case amounts to determining
the marginal density of $\bc Y$, and evaluate that density at $\vec
y_{\text{obs}}$.  To follow this course we would first determine the joint
density of $\bc U$ and $\bc Y$, written $f_{\bc U,\bc Y}(\vec u,\vec y)$,
then integrate this density with respect to $\vec u$ to create the marginal
density, $f_{\bc Y}(\vec y)$, and finally evaluate this marginal density at
$\vec y_{\text{obs}}$.
...
mixed models, described in Sect.~\ref{sec:general}, are often used to
...
\end{equation}
The actual evaluation of the sparse Cholesky factor, $\vec L_\theta$,
...
%% LaTeX FIXME \widehat{\vec\beta} failing inside \subsection{..}
\subsection{Determining $r^2_\theta$ and $\hat{\vec\beta}_\theta$}% FIXME?
\label{sec:betahat}
...
function may be called many, many times for different values of the
...
(df1 <- as(fm08, "function"))# calls hidden mkdevfun() -->  =?= mkdevfun(fm08)
@
...
constructions.
<<mkdevfun2>>=
df2 <- as.function(fm08, control=list(compDev=FALSE))## needs lme4-devel (>= 2018-02-08)
   ## or mkLmerDevfun(fr, ... ???) ## was mkdevfun(fm08, compDev=FALSE)
head(df2)
@
...
<<theta>>=
theta <- c(1, 0, 1)
Lambda <- getME(fm08, "Lambda")
Lambda@x <- getME(fm08, "Lind")
@
...
%% was  attach(env(fm08))
<<attachrho>>=
attach(as.environment(fm08@pp), name="fm08@pp")
attach(as.environment(fm08@resp), name="fm08@resp")
@
...
<<updateLambda>>=
str(getME(fm08, "Lambda"))
str(getME(fm08, "Lind"))
##_FIXME __ TODO __ of course, cannot use getME() in   LHS <- ...
##_F getME(fm08, "Lambda")@x[] <- c(1,0,1)[getME(fm08, "Lind")]
##_F str(getME(fm08, "Lambda")@x)
Ut <- crossprod(getME(fm08, "Lambda"), getME(fm08, "Zt"))
@
...
<<updateL>>=
L <- update(getME(fm08, "L"), Ut, mult = 1)
@
Then we evaluate \code{RZX} and \code{RX} according to (\ref{eq:RZXdef}) and
(\ref{eq:RXdef})
<<RZXandRX>>=
RZX <- solve(L, solve(L, crossprod(getME(fm08, "Lambda"),
                                   getME(fm08, "Zt") %*% getME(fm08, "X")),
                      sys = "P"),
             sys = "L")
#RX <- chol(crossprod(getME(fm08, "X")) - crossprod(RZX))
@
...
we evaluate
<<cu>>=
#cu <- solve(L, solve(L, crossprod(getME(fm08, "Lambda"), getME(fm08, "Utr")), sys = "P"), sys = "L")
#cbeta <- solve(t(RX), getME(fm08, "Vtr") - crossprod(RZX, cu))
@
...
<<wrapup>>=
#mu <- gamma <- as.vector(crossprod(Ut, u) + getME(fm08, "X") %*% fixef)
#prss <- sum(c(getME(fm08, "y") - mu, as.vector(u))^2)
...
\code{fm08@setPars}, ---FIXME--- %% <<<<<<<<<<<<<<<<
is a slightly modified version of what is shown
...
An extended decomposition (\ref{eq:bigdecomp}) provides the
