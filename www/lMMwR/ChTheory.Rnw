...
The pseudo-data representation of the PLS problem, (\ref{eq:PLSLMM}), becomes
\begin{equation}
  \label{eq:PLSLMMP}
  \tilde{\vec u}=\arg\min_{\vec u}\left\|
    \begin{bmatrix}
...
logarithm of the likelihood, now becomes
\begin{displaymath}
...
to $\vec\theta$ to determine the MLE, $\widehat{\vec\theta}$.  The
