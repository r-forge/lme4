...
constant multiple of $f_{\bc U|\bc Y}(\vec u|\vec y)$.  The last part
...
\begin{equation}
  ...
  \exp\left(-\frac{\left\|\vec y-\vec X\vec\beta-\vec Z\Lambda_\theta\vec u\right\|^2+\|\vec u\|^2}{2\sigma^2}\right) .
\end{equation}
...
P$.  That is, the matrix $\vec P$ is only a notational convenience
only.  The permutation is determined from the structure of
...
numeric phase, which is often much faster than the symbolic phase.
...
must solve (\ref{eq:LMMPLSsolP}) for $\tilde{\vec u}$.  However, this
is the key step in computational methods in the \package{lme4}
...
because $\vec L_\theta$ is triangular.  In Sect.~\ref{sec:stepByStep}
we will describe the steps in determining this solution.
...
logarithm of the likelihood, can now be expressed as
\begin{displaymath}
...
of the parameters are those that minimize this deviance.  Note that
evaluating the determinant, $|\vec L_\theta|$, which may seem
formidable, is not. Because $\vec L_\theta$ is triangular, its
determinant is simply the product of its diagonal elements.
...
is just an extension of the penalized least squares problem.  Let
...
Numerical optimization of $\tilde{d}(\vec\theta|\vec y)$ with respect
to $\vec\theta$ determines the MLE, $\widehat{\vec\theta}$.  The
MLEs for the other parameters, $\widehat{\vec\beta}$ and
$\widehat{\sigma}$, are the corresponding conditional estimates
evaluated at $\widehat{\vec\theta}$.
...
minimize $d_R(\vec\theta,\sigma|\vec y)$.
...
As we have seen, an object returned by \code{lmer} contains an
...
Consider our model \code{fm8}, fit as
<<fm8reduxshow,eval=FALSE>>=
fm8 <- lmer(Reaction ~ 1 + Days + (1 + Days|Subject),
            sleepstudy, REML = 0, verbose = TRUE)
@ 
<<fm8redux,echo=FALSE>>=
...
$\vec\theta$, (\code{theta}), the relative covariance factor,
...
(\code{fixef}).  The permutation represented by $\vec P$ is part of
the representation of $\vec L$.

Although the model matrices, $\vec X$ (\code{X}) and $\vec Z\trans$
(\code{Zt}), and the response vector, $\vec y$ (\code{y}), are
...
To provide easy access to the objects in the environment of \code{fm8} we
attach it to the search path.
<<attachrho>>=
attach(env(fm8))
@
Please note that this is done here for illustration only.  The
practice of attaching a list or a data frame or, less commonly, and
environment in an \R{} session is overused, somewhat dangerous
(because of the potential of forgetting to detach it later) and
discouraged.  The better practice is to use the \code{with} function
to gain access to names in such composite objects.  For this section
of code, however, using \code{with} or \code{within} would quickly
become very tedious.

To update the matrix $\Lambda_\theta$ to a new value
of $\vec\theta$ we need to know which of the non-zeros in
$\Lambda$ are updated from which elements of $\vec\theta$.  Recall
that the dimension of $\vec\theta$ is small (3, in this case) but
$\Lambda$ is potentially large ($18\times18$ with $54$ non-zeros).
The environment \code{rho} contains an integer vector \code{Lind} that
maps the elements of \code{theta} to the non-zeros in \code{Lambda}.
...
$\Lambda$ and forming the product
