...
determining this solution.  First, though, we should show that the
...
Technically, it is the absolute value of $|\vec L_\theta|$ that should
be used in (\ref{eq:tranJac}) and (\ref{eq:hintegral}) but $\vec
L_\theta$ is evaluated in such a way that the diagonal elements are
all positive and hence its determinant is positive.  The determinant
of $\vec L_\theta$ cannot be zero because $\vec L_\theta$ is the
factor of a positive semidefinite matrix plus the identity and, as
such, must be non-singular.  The more important question, which is
whether it stays safely non-singular computationally, is also answered
in the affirmative.
...
Problem~\ref{prob:downdate}).  In practice, the downdated matrix can
...
$\widehat{\vec\beta}$ and, although the residual vector $\vec y_{\text{obs}}-\vec
X\vec\beta$ is an $n$-dimensional vector, the residual at
$\widehat{\vec\theta}$ satisfies $p$ linearly independent constraints,
$\vec X\trans(\vec y_{\text{obs}}-\vec X\widehat{\vec\beta})=\vec 0$. That is, the
residual at $\widehat{\vec\theta}$ is the projection of the observed
response vector, $\vec y_{\text{obs}}$, into an $(n-p)$-dimensional linear subspace
...
discouraged.  The preferred practice is to use the \code{with} function
...
become very tedious and, in this case, we use \code{attach} instead.
...
The last step is detach the environment of \code{fm8} from the search list
<<detachenv>>=
detach()
@ 
...
\code{fm8@setPars}, is a slightly modified version of what is shown
...
\begin{equation}
  ...
  =n\log(2\pi\sigma^2)+\log(|\vec L_{\theta,\beta}|^2)+
