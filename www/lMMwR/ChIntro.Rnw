...
is estimated as \Sexpr{sprintf("%.2f", unlist(VarCorr(fm01)))} g$^2$
in the REML fit and as
\Sexpr{sprintf("%.2f", unlist(VarCorr(fm01ML)))} g$^2$
in the ML fit.  The corresponding standard deviations
are \Sexpr{sprintf("%.2f", sqrt(VarCorr(fm01)[["Batch"]][1,1]))} g for the
REML fit and \Sexpr{sprintf("%.2f", sqrt(VarCorr(fm01ML)[["Batch"]][1,1]))}
g for the ML fit.
...
$\widehat{\vec\beta}=\widehat{\vec\beta}_{\widehat\theta}$, $\tilde{\vec u}_{\widehat{\theta}}$ and
...
<<fm01MLverb>>=
fm01ML <- lmer(Yield ~ 1|Batch, Dyestuff, REML = FALSE, verbose = TRUE)
@ 
The algorithm converges in 6 iterations to a profiled deviance of
\Sexpr{sprintf("%.5f", deviance(fm01ML))}
at $\theta=$\Sexpr{sprintf("%.6f", fm01ML@getPars())}.
...
print(image(env(fm01)$Lambda, sub=NULL, xlab=NULL, ylab=NULL))
@   
...
print(image(env(fm01)$Zt, sub=NULL))
@   
...
\code{image(env(fm01ML)\$L}).  The vectors $\vec u$ and $\vec b$ and
the matrix $\vec X$ have the same names in \code{env(fm01ML)}.  The
...
\begin{figure}[tbp]
  \sidecaption[t]
  \setkeys{Gin}{width=1.5in}
<<fm01predqq,fig=TRUE,echo=FALSE,results=hide,width=3.5,height=3.5>>=
...
@
(Fig.~\ref{fig:fm01predqq}), where the intervals are plotted versus
