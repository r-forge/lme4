...
print(densityplot(lme4:::varianceProf(pr01),
...
takes an optional argument, \code{condVar = TRUE}, which adds these
...
print(dotplot(ranef(fm01ML, condVar =TRUE), strip = FALSE))
@
...
print(qqmath(ranef(fm01ML, condVar =TRUE), strip = FALSE))
@
\caption{95\% prediction intervals on the random effects in
...
\item[$\vec\gamma$] the $n$-dimensional linear predictor,
  \begin{displaymath}
