...
\begin{equation}
  \label{eq:Lmat}
  \bm L_\theta\bm L_\theta\trans=\bm U_\theta\trans\bm U_\theta+\bm I_q .
\end{equation}
...
considering this is trivial because $\bm U_\theta=\bm Z(\theta\bm
I_6)=\theta\bm Z$ and $\bm U_\theta\trans\bm U_\theta+\bm I_6=(1 +
5\theta^2)\bm I_6$ so a suitable $\bm L_\theta=\sqrt{1 + 5\theta^2}\bm
I_6$.  The calculations are straightforward in this case.  However,
the \code{lmer} function has been used to fit linear mixed models for
which the number of rows and the number of columns in $\bm U_\theta$
were both more than one million and the structure of $\bm
U_\theta\trans\bm U_\theta+\bm I_q$ was much more complex than a
simple multiple of an identity matrix.  The ability to compute this
factorization reasonably quickly for many different values of
$\bm\theta$ in cases like this is what makes the methods in the
\package{lme4} package feasible.
...
\begin{equation}
  \label{eq:PLRSS}
  r^2_\theta = \min_{\bm u,\bm\beta}\left[\|\bm y-\bm U_\theta\bm u-\bm
  X\bm\beta\|^2+\|\bm u\|^2\right]
\end{equation}
...
\begin{equation}
  \label{eq:profDev}
  -2\tilde{\ell}(\bm\theta)=\log(|\bm L_\theta|^2)+n\left[1+
    \log\left(\frac{2\pi r_\theta^2}{n}\right)\right] .
\end{equation}
...
$\widehat{\theta}=\Sexpr{round(env(fm1ML)[["theta"]], 3)}$.
...
L_\theta|^2)$ and $r^2_\theta$, the minimum penalized residual sum
...
res$lprssRE <- (n - 1) * (1 + log(2 * pi * res$prss/(n - 1)))
...
model]{The profiled deviance and its components as a function of
  ...
  panel labeled \code{ldL2} is $\log(|\bm L_\theta|^2)$ and the
  panel labeled \code{lprss} is $n[1+\log(2\pi r^2_\theta/n)]$.}
  \label{fig:DyeML}
\end{figure}
we show the deviance as a function of $\theta$ (bottom panel) and the
two additive terms, $\log(|\bm L(\theta)|^2)$ and $n[1+\log(2\pi
r_\theta^2/n)]$ that occur in (\ref{eq:profDev}).  In this figure we can see
that the logarithm of the determinant of $\bm L$ increases as $\theta$
...
The other term, $n[1+\log(2\pi r_\theta^2/n)]$, depends on $\theta$
only through the minimum penalized residual sum of squares,
$r_\theta^2$.  In Figure~\ref{fig:prssCom}
\begin{figure}[tbp]
  \centering
...
print(xyplot(prss + usqr + rss ~ theta,
	     ...
	     type = c("g","l"), layout = c(1,3)))
@   
  \caption[Components of $r_\theta^2$]{Components of the penalized
    sum of square residuals, $r_\theta^2$ (labeled \code{prss}).
    The residual sum of squares, $\|\bm y-\bm U(\theta)\tilde{\bm
      u}_\theta - \bm X\widehat{\bm\beta}_\theta\|^2$, where
    $\tilde{\bm u}$ and $\widehat{\bm\beta}_\theta$ jointly minimize the
    penalized least squares criterion, is labeled \code{wrss}.  The
    penalty term, $\|\tilde{\bm u}\|^2$, is labeled \code{usqr}.}
  \label{fig:prssCom}
\end{figure}
we plot $r_\theta^2$ (labeled \code{prss}) and its two components
$\|\tilde{\bm u}_\theta\|^2$ (labeled \code{usqr}) and $\|\bm y-\bm
U_\theta\tilde{\bm u}_\theta-\bm X\widehat{\bm\beta}_\theta\|^2$
...
\begin{equation}
  ...
  \|\bm y-\bm U_\theta\bm u-\bm X\bm\beta\|^2+\|\bm u\|^2\right] .
\end{equation}
...
\begin{equation}
  ...
  \begin{bmatrix}
    \bm U_\theta\trans\bm U_\theta + \bm I & \bm U_\theta\trans\bm X\\
    \bm X\trans\bm U_\theta         & \bm X\trans\bm X
  \end{bmatrix}
  \begin{bmatrix}
    \tilde{\bm u}_\theta\\
    \widehat{\bm\beta}_\theta
  \end{bmatrix} =
  \begin{bmatrix}\bm U_\theta\trans\bm y\\\bm X\trans\bm y\end{bmatrix} .
\end{equation}

In Figure~\ref{fig:prssCom} the plot of the sum of squared residuals,
$\|\bm y-\bm U_\theta\tilde{\bm u}_\theta-\bm
X\widehat{\bm\beta}_\theta\|^2$, labeled \code{wrss}, versus $\theta$
...
\begin{equation}
  \label{eq:wrssbounds}
  \min_{\bm u,\bm\beta}\|\bm y-\bm U_\theta\bm u-\bm X\bm\beta\|^2\le
  \|\bm y-\bm U_\theta\tilde{\bm u}_\theta-\bm X\widehat{\bm\beta}\|^2
  \le \min_{\bm\beta}\|\bm y-\bm X\bm\beta\|^2 .
\end{equation}

It turns out that these bounds also apply to the penalized residual
sum of squares, labeled \code{prss} in
Figure~\ref{fig:prssCom}. We can either reason that
\begin{equation}
  ...
  \le \min_{\bm\beta}\|\bm y-\bm X\bm\beta\|^2 .
\end{equation}
or check the pattern in the plot of $\|\tilde{\bm u}_\theta\|^2$,
labeled \code{usqr} in  Figure~\ref{fig:prssCom}.  This term is zero when
$\theta=0$ and, as $\theta\rightarrow\infty$, it returns to
zero.
...
bounds, because it is an increasing function of $r_\theta^2$, which
...
have $\widehat{\theta}=\infty$.

We can, however, have $\widehat{\theta}=0$, which is exactly what occurs
...
model]{The profiled deviance and its components as a function of
  ...
  panel labeled \code{ldL2} is $\log(|\bm L_\theta|^2)$ and the panel
  labeled \code{lprss} is $n[1+\log(2\pi r_\theta^2)/n)]$.  The
  panel labeled \code{prss} is $r_\theta^2$, with components
...
$\log(|\bm L_\theta|^2)$ (\code{ldL2}) and $n[1+\log(2\pi r^2/n)]$
(\code{lprss}), and $\theta$ are similar for the two models.  In fact,
$\log(|\bm L_\theta|^2)$ versus $\theta$ is exactly the same for the
two models.  Although the shape of the relationship between
$n[1+\log(2\pi r^2/n)]$ and $\theta$ in Figure~\ref{fig:Dye2ML} is
similar to that in Figure~\ref{fig:DyeML}, the range on the vertical
axis is very different.  Recall that for the \code{Dyestuff} model,
the range of the penalized residual sum of squares, $r_\theta^2$, was
\begin{equation}
  \label{eq:rsqrrange}
...
\begin{equation}
  ...
  \le n\left[1+\log\left(\frac{2\pi r_\theta^2}{n}\right)\right] \le
...
\end{equation}
and this range is too narrow to produce an estimate $\widehat{\theta}$
that is greater than zero.

\subsection{Interpretation of the deviance components}
\label{sec:devcompinterp}

The two components of the profiled deviance, $\log(|\bm L_\theta|^2)$
and $n[1 + \log(2\pi r_\theta^2/n)]$, can be considered as measuring
...
L_\theta|^2)$.  The simplest model, corresponding to $\theta = 0$,
...
or by the penalized residual sum of squares, $r_\theta^2$, increases
...
In the case of the \code{Dyestuff2} data the fidelity component does
not change sufficiently with increasing $\theta$ to compensate for the
increased model complexity and the optimal value of $\theta$ is at the
boundary, $\widehat{\theta}=0$.
...
criterion that is labeled \code{REML} in the display of a fitted
...
\begin{displaymath}
  \begin{bmatrix}
    \bm U_\theta\trans\bm U + \bm I & \bm U_\theta\trans\bm X\\
...
We can extend the Cholesky factor of $\bm U_\theta\trans\bm U+\bm I_q$ to
...
\begin{equation}
  ...
  \end{bmatrix} =
  \begin{bmatrix}
    \bm U_\theta\trans\bm U_\theta+\bm I_q & \bm U_\theta\trans\bm X\\
    \bm X\trans\bm U_\theta         & \bm X\trans\bm X
  \end{bmatrix}
\end{equation}
of the matrix defining the system of equations (\ref{eq:PLSjointSol}).
In (\ref{eq:bigChol}) $\bm R_X$ is a $p\times p$ upper triangular
matrix.  The $q\times p$ matrix $\bm R_{ZX}$ is the solution to the
triangular system
\begin{equation}
  \label{eq:RZX}
  \bm L_\theta\bm R_{ZX}=\bm U_\theta\trans\bm X
\end{equation}
...
\begin{equation}
  \label{eq:REMLdev}
  -2\tilde{\ell}_R(\theta)=\log(|\bm L_\theta|^2|\bm R_X|^2)+(n-p)\left[1+
    \log\left(\frac{2\pi r_\theta^2}{n-p}\right)\right] .
\end{equation}
...
the residual vector.  The name ``degrees of freedom'' reflects that
...
the residual vector must satisfy.
...
Another characteristic that many consider to be important is that the
...
res <- within(res, ldSum <- ldL2 + ldRX2)
...
model]{Components of the REML criterion for the model fit to the
  ...
  $\log(|\bm L_\theta|^2)$, as before, and the component labeled \code{ldRX2}
  is $\log(|\bm R_X|^2)$.  Their sum is the component labeled
  \code{ldSum}. The component labeled \code{lprssRE} is $(n-1)[1 +
  \log(2\pi r_\theta^2/(n-p))]$.  The REML criterion, labeled \code{REML}, is
...
noted previously, the component $\log(|\bm L_\theta|^2)$ increases with
increasing $\theta$.  The other two additive components, $\log(|\bm
R_X|^2)$ and $(n-1)[1 + \log(2\pi r_\theta^2/(n-p))]$ decrease with
increasing $\theta$.  The range of the fidelity component
\begin{equation}
  \label{eq:devcomp3range}
  \Sexpr{sprintf("%.3f", devcomp1RElow)}
  \le (n - 1)\left[1+\log\left(\frac{2\pi r_\theta^2}{n-1}\right)\right] \le
...
L_\theta|^2)$, combined with the contribution of the decreasing component,
...
The fidelity component, $(n-1)[1 + \log(2\pi r_\theta^2/(n-p))]$, labeled
...
\sigma^2\bm\Lambda_\theta\bm\Lambda_\theta\trans)$ and the
...
0,\sigma^2\bm\Lambda_\theta\bm\Lambda_\theta\trans)$.
...
\begin{equation}
  \label{eq:condmeanU}
  (\bm U_\theta\trans\bm U+\bm I_q)\bm\mu_{\bc U|\bc Y=\bm y} =
  \bm L_\theta\bm L_\theta\trans\bm\mu_{\bc U|\bc Y=\bm y}=
  \bm U_\theta\trans\left(\bm y - \bm X\bm\beta\right) .
\end{equation}
...
\begin{equation}
  \label{eq:condmeanB}
  \bm\mu_{\bc B|\bc Y=\bm y} = \bm\Lambda_{\hat\theta}
  \tilde{\bm u}_{\hat\theta} .
\end{equation}
...
considering the matrices $\bm\Lambda_\theta$ and $\bm L_\theta$ are
both diagonal --- in fact, they are multiples of the identity matrix
--- so it is easy to compute $\left(\bm L_\theta\bm
  L_\theta\trans\right)^{-1}$.  In general, though, it is not easy
...
in estimating the amount of variability in the response induced by
the selection of experimental or observational units.  The parameter
$\sigma^2$ in the model
\begin{equation}
  \label{eq:LMM}
  \bc B\sim\mathcal{N}\left(\bm 0,
    \sigma^2\bm\Lambda_\theta\bm\Lambda_\theta\trans\right),\:
...
$\theta$ for the model \code{fm1}.  The ratio
of the variance estimates,
\begin{displaymath}
 \widehat{\sigma^2}_{ML}/\widehat{\sigma^2}_R = \frac{n-p}{n}=1-\frac{p}{n}
\end{displaymath}
does not change with $\theta$ and here even the ratio of the estimated
standard deviations is nearly constant.

In cases like this where we have just two ``variance components'',
$\sigma^2$ and $\sigma_B^2$, we can examine the deviance or the REML
criterion as a functions of $\sigma$ and $\sigma_B$.  In
Figure~\ref{fig:devcontoursfm1}
\begin{figure}[tbp]
  \centering
...
profzs <- sqrt(profsbd[,2]-deviance(fm1ML)) *
    ifelse(sigmas < attr(VCML, "sc"), -1, 1)               
profzsb <- sqrt(profsd[,2]-deviance(fm1ML)) *
    ifelse(sigB < attr(VCML$Batch, "stddev"), -1, 1)
...
that the profiled deviance is not at all close to being a quadratic
function of $\sigma$ or $\sigma_B$.  Expressed as a function of
$\sigma^2$ and $\sigma_B^2$ (Figure~\ref{fig:profileddevsqfm1}),
\begin{figure}[tbp]
  \centering
...
print(xyplot(profz ~ x|par,
             data.frame(profz = c(profzs, profzsb),
                        x = c(sigmas^2, sigB^2),
                        par = gl(2, length(sigmas))),
             strip = function(..., factor.levels)
             strip.default(..., factor.levels = c(expression(sigma^2),
             ...
             scales = list(x = list(relation = "free", axs = 'i'))))
@   
\caption{Profile z statistic in model \code{fm1} for $\sigma^2$ and
  $\sigma_B^2$.}
  \label{fig:profzfm1}
\end{figure}
