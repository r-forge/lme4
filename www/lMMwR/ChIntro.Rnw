...
Fig.~\ref{fig:Dyestuffdot} than we can from
numerical summaries.
...
in section~\ref{sec:assessRE} we show methods of characterizing
...
deviance~(\ref{eq:LMMprofdev}).  We determine this value by numerical
...
(Fig.~\ref{fig:fm1Lambdaimage}).
\begin{figure}[tbp]
  \sidecaption[t]
  \setkeys{Gin}{width=2in}
<<fm1Lambdaimage,fig=TRUE,echo=FALSE,width=2.5,height=2.5>>=
...
transpose is shown in Fig.~\ref{fig:fm1Ztimage}, consists of the
...
@   
\caption[Profile zeta plots of the parameters in model \code{fm1ML}]
{Signed square root, $\zeta$, of the likelihood ratio test statistic
  for each of the parameters in model \code{fm1ML}.  The vertical
...
(Fig.~\ref{fig:fm1prof}) are obtained with
<<fm1profshow>>=
...
\code{fm1ML}] {Profiled deviance, on the scale $|\zeta|$, the square
  root of the change in the deviance, for each of the parameters in
  model \code{fm1ML}.  The intervals shown are 50\%, 80\%, 90\%, 95\%
...
parameter (Fig.~\ref{fig:fm1absprof}), obtained by adding the
...
A profile zeta plot, such as Fig.~\ref{fig:fm1prof}, shows us the
...
we can perform reliable statistical inference based on the parameter's
...
In Fig.~\ref{fig:fm1prof} the profile zeta plot for $\log(\sigma)$
\begin{figure}[tb]
  \centering
...
$\sigma^2$ in model \code{fm1ML}]{Signed square root, $\zeta$, of the
...
$\sigma_1^2$ in model \code{fm1ML}]{Signed square root, $\zeta$, of the
...
\code{fm1}]{Profile pairs plot for the parameters in model \code{fm1}.
...
splom(pr1)
@
and shown in Fig.~\ref{fig:fm1profpair} shows the profile traces
...
random effects it can be helpful to think of them this way.  However,
it can also be misleading.  As we have stated, the random effects are
not, strictly speaking, parameters---they are unobserved random
variables.  We don't estimate the random effects in the same sense
that we estimate parameters.  Instead, we consider the conditional
distribution of $\bc B$ given the observed data.  That is, we consider
$(\bc B|\bc Y=\vec y)$.
...
@ 
(Fig.~\ref{fig:fm1preddot}), which provides linear spacing
\begin{figure}[tbp]
  \sidecaption[t]
  \setkeys{Gin}{width=1.5in}
<<fm1predqq,fig=TRUE,echo=FALSE,results=hide,width=3.5,height=3.5>>=
print(qqmath(ranef(fm1ML, postVar=TRUE), strip = FALSE))
@   
\caption{95\% prediction intervals on the random effects in
  \code{fm1ML} versus quantiles of the standard normal distribution.}
  \label{fig:fm1predqq}
\end{figure}
...
@
(Fig.~\ref{fig:fm1predqq}), where the intervals are plotted versus
...
deviance function with respect each parameter and apply a signed
...
individual parameters.  Profile pairs plots allow us to visualize the
pairwise dependence of parameter estimates and two-dimensional
marginal confidence regions.
...
\begin{prob}\label{prS1}
  ...
  \code{Rail} data (Fig.~\ref{fig:Raildot}) from the \package{MEMSS}
  package.  Note that if you used \code{data} to access this data set
  then you must use
<<helpRail,eval=FALSE>>=
...
\begin{prob}
  ...
  conditional modes of the the random effects.
\end{prob}
\begin{prob}
...
\begin{prob}
  ...
  is there a good normal approximation?
\end{prob}
\begin{prob}
  Create a profile pairs plot for this model.  How does the shape of
  the contours in this model correspond to those in
  Fig.~\ref{fig:fm1profpair}?
\end{prob}
\begin{prob}
  ...
  in this model compared to those in model \code{fm1} for the
  ...
  plot than those in Fig.~\ref{fig:fm1preddot}?
\end{prob}
