...
is estimated as \Sexpr{sprintf("%.2f", unlist(VarCorr(fm1)))} g$^2$
in the REML fit and as
\Sexpr{sprintf("%.2f", unlist(VarCorr(fm1ML)))} g$^2$
in the ML fit.  The corresponding standard deviations
are \Sexpr{sprintf("%.2f", sqrt(VarCorr(fm1)[["Batch"]][1,1]))} g for the
...
of the residuals is \Sexpr{round(attr(VarCorr(fm1), "sc"), 2)} g and
...
parameter is \Sexpr{round(fixef(fm1)[1], 2)} g (equality is again a
consequence of the simple model and balanced data set).  The standard
error of the intercept estimate is
\Sexpr{round(coef(summary(fm1))[1,2], 2)} g for the REML fit and
\Sexpr{round(coef(summary(fm1ML))[,2], 2)} g for the ML fit.
...
describe such as situation as providing a good normal approximation
...
\begin{prob}\label{prS1}
  Check the documentation, the structure (\code{str}) and a summary of the
...
\end{prob}
\begin{prob}
  Create a profile pairs plot for this model.  Does the shape of the
  deviance contours in this model mirror those in
