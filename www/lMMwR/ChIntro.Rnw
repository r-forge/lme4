...
\Sexpr{round(summary(fm1)@coefs[1,2], 2)} gm. for the REML fit and
\Sexpr{round(summary(fm1ML)@coefs[,2], 2)} gm. for the ML fit.
...
$\bm\Sigma(\bm\theta)$.  The notation indicates that this symmetric
...
\begin{equation}
  ...
  \bc{B}\sim\mathcal{N}(\bm 0,\bm\Sigma(\bm\theta)) .
\end{equation}
...
a sparse matrix in the fitted model object in a ``slot'' called
\code{Zt}.
<<fm1Zt,eval=FALSE>>=
fm1@Zt
@ 
<<fm1Ztreal,echo=FALSE>>=
tt <- fm1@Zt
rownames(tt) <- LETTERS[1:nrow(tt)]
tt
@
...
$\bm\Sigma(\bm\theta)$ is a non-negative multiple of $\bm I_q$, the
$q\times q$ identity matrix.  Although we will defer until later the
discussion of the exact form of $\bm\Sigma(\bm\theta)$ for other
models, we will now introduce a transformation of
$\bm\Sigma(\bm\theta)$ that we will use throughout.
...
matrix, say $\bm\Lambda(\bm\theta)$, such that when it is multiplied
by its transpose it produces the original matrix,
$\bm\Sigma(\bm\theta)$.  This is not quite like ``squaring'' the
matrix because the factor, $\bm\Lambda(\bm\theta)$, is multiplied by
its transpose, so as to create a product that is symmetric, as
$\bm\Sigma(\bm\theta)$ must be.

It turns out that we can simplify many subsequent formulas if we
define $\bm\Lambda(\bm\theta)$ to be a multiple of the square-root
factor, which we call the \emph{relative covariance factor}, defined
to satisfy
\begin{equation}
  \label{eq:relcovfac}
  \bm\Sigma(\bm\theta)=\sigma^2\bm\Lambda(\bm\theta)
  \bm\Lambda(\bm\theta)\trans .
\end{equation}
The symbol $\trans$ denotes the transpose of a matrix and $\sigma^2$
is the common scale parameter defined in (\ref{eq:LMMconddist}).

The term ``relative'' indicates that $\bm\Lambda(\bm\theta)$ is the
...
$\bm\Lambda(\bm\theta)$ it ends up being lower triangular and
corresponds to the left Cholesky factor of
$\bm\Sigma(\bm\theta)/\sigma^2$.  A left Cholesky factor is often
...
random-effects term, $\bm\Sigma(\bm\theta)$ and
$\bm\Lambda(\bm\theta)$ are both multiples of $\bm I_6$, the identity
matrix of size six.  Furthermore, $\bm\theta$ is one-dimensional so we
will write it as $\theta$.  We set
\begin{equation}
  \label{eq:lambdatheta}
  \bm\Lambda(\theta) = \theta\bm I_6
\end{equation}
...
\begin{equation}
  \label{eq:Umat}
  \bm U(\bm\theta)=\bm Z\bm\Lambda(\bm\theta),
\end{equation}
...
Because $\bm L(\bm\theta)$ is square and lower triangular, its
...
\begin{equation}
  \label{eq:PLRSS}
  r^2(\bm\theta) = \min_{\bm u,\bm\beta}\|\bm y-\bm U\bm u-\bm
  X\bm\beta\|^2+\|\bm u\|^2
\end{equation}
...
Before describing the details of the calculation of $r^2(\bm\theta)$
...
<<fm1MLverb>>=
fm1ML <- lmer(Yield ~ 1 + (1 |Batch), Dyestuff, verbose = 1, REML = FALSE, control=list(algorithm="nlminb"))
@ 
...
$\hat{\theta}=\Sexpr{round(getPars(fm1ML@rCF), 3)}$.

Some of the components of the deviance are available in the
\code{deviance} slot of the fitted model.
<<fm1MLdev>>=
round(fm1ML@deviance[c(1,3,7,9,10)],2)
@ 
The elements named \code{"ldL2"}, \code{"usqr"} and \code{"wrss"} are
$\log(|\bm L|^2)$, $\|\bm u\|^2$ and $\|\bm y-\bm U\bm u-\bm
X\bm\beta\|^2$, respectively.  (The name \code{"wrss"} stands for
``weighted residual sum of squares''.  In some mixed models weights
are applied to the residuals before they are squared and summed.)  The
element named \code{"pwrss"} is $r^2(\bm\theta)$, the minimum penalized
weighted residual sum of squares.  We can evaluate the deviance as
<<fm1MLdev2>>=
dev <- fm1ML@deviance[c(1,3,7,9,10)]
n <- nrow(model.frame(fm1ML))
(calc <- dev["ldL2"] + n * (1 + log(2 * pi * dev["pwrss"]/n)))
all.equal(unname(calc), unname(dev["ML"]))
@ 
...
$r^2(\theta)$.  In Figure~\ref{fig:pwrssCom}
\begin{figure}[tbp]
  \centering
<<pwrssCom,fig=TRUE,echo=FALSE,>>=
print(xyplot(pwrss + usqr + wrss ~ theta,
             res, xlab = expression(theta), ylab = NULL, outer = TRUE,
             scales = list(y = list(relation = "free", rot = 0),
                           x = list(axs = 'i')),
	     strip = FALSE, strip.left = TRUE,
...
(fm2ML <- lmer(Yield ~ 1 + (1 |Batch), Dyestuff2, verbose = 1,
               REML = FALSE, control = list(algorithm = "nlminb")))
@ 
...
print(xyplot(deviance + ldL2 + lprss + pwrss + usqr + wrss ~ theta,
...
model]{The profiled deviance and its components as a function of
  ...
  panel labeled \code{pwrss} is $r^2(\theta)$, with components
  \code{usqr} and \code{wrss}.}
  \label{fig:Dye2ML}
\end{figure}
...
least squares problem, (\ref{eq:PLSjointSol}). Recall that $\tilde{\bm
  u}(\bm\theta)$ and $\widehat{\bm\beta}(\bm\theta)$ are the solutions
to
\begin{displaymath}
  \begin{bmatrix}
    \bm U\trans\bm U + \bm I & \bm U\trans\bm X\\
...
\begin{displaymath}
  \bm U(\bm\theta)=\bm Z\bm\Lambda(\bm\theta) .
\end{displaymath}
Furthermore, the Cholesky factor, $\bm L(\bm\theta)$, is the sparse,
lower-triangular matrix that satisfies (\ref{eq:Lmat}),
\begin{displaymath}
  \bm L(\bm\theta)\bm L\trans(\bm\theta)=\bm U\trans(\bm\theta)\bm U(\bm\theta)+\bm I_q .
\end{displaymath}
...
\begin{equation}
  \label{eq:REMLdev}
  -2\tilde{\ell}_R(\bm\theta)=\log(|\bm L|^2|\bm R_X|^2)+(n-p)\left[1+
...
the conditional estimates, $\widehat{\bm\beta}(\bm\theta)$, satisfying
...
res <- within(res,
          {
              lprssRE <- (n - 1) * (1 + log(2 * pi * pwrss/(n - 1)))
              ldSum <- ldL2 + ldRX2
          })
...
\begin{equation}
  \label{eq:thetacomp}
  \widehat{\theta_R}=
  \Sexpr{sprintf("%.4f", getPars(fm1@rCF))} >
    \Sexpr{sprintf("%.4f", getPars(fm1ML@rCF))}
...
\sigma^2\bm\Lambda(\bm\theta)\bm\Lambda\trans(\bm\theta))$ and the
...
\begin{equation}
  \label{eq:BLambdaU}
  \bc B=\bm\Lambda(\bm\theta)\bc U,
\end{equation}
provides the required distribution $\bc B\sim\mathcal{N}(\bm
0,\sigma^2\bm\Lambda(\bm\theta)\bm\Lambda\trans(\bm\theta)$.
...
\begin{equation}
  \label{eq:condmeanU}
  (\bm U\trans(\bm\theta)\bm U+\bm I_q)\bm\mu_{\bc U|\bc Y=\bm y} =
  \bm L(\bm\theta)\bm L\trans(\bm\theta)\bm\mu_{\bc U|\bc Y=\bm y}=
  \bm U\trans(\bm\theta)\left(\bm y - \bm X\bm\beta\right) .
\end{equation}
...
parameters, $\widehat{\bm\beta}(\bm\theta)$, and the conditional mean
of the spherical random effects, $\tilde{\bm u}(\bm\theta)$, for a
...
\begin{equation}
  ...
  \left(\bm L(\bm\theta)\bm L\trans(\bm\theta)\right)^{-1},
\end{equation}
hence 
\begin{equation}
  \label{eq:condVarB}
...
--- so it is easy to compute $\left(\bm L(\theta)\bm
  L\trans(\bm\theta)\right)^{-1}$.  In general, though, it is not easy
...
\begin{equation}
  ...
  \bc B\sim\mathcal{N}\left(\bm 0,
    \sigma^2\bm\Lambda(\bm\theta)\bm\Lambda\trans(\bm\theta)\right),\:
...
\begin{equation}
  \label{eq:LMMsigmaML}
  \widehat{\sigma^2}_{\mathit{ML}}=\frac{r^2(\bm\theta)}{n}
\end{equation}
and the conditional REML estimate is
\begin{equation}
  \label{eq:LMMsigmaREML}
  \widehat{\sigma^2}_{\mathit{R}}=\frac{r^2(\bm\theta)}{n-p} .
\end{equation}
...
print(xyplot(sigmaML + sigmaREML ~ theta, res,
             outer = FALSE,
...
\begin{equation}
  ...
  \min_{\bm u,\bm\beta}\left(
    \left\|\bm y - \bm Z\bm\Lambda(\bm\theta)\bm u -
      \bm X\bm\beta\right\|^2 + \|\bm u\|^2\right)
\end{equation}
as the vectors, $\widehat{\bm\beta}(\bm\theta)$ and $\tilde{\bm u}$,
that satisfy
\begin{equation}
  \label{eq:plsSol}
  \begin{bmatrix}
    \bm\Lambda(\bm\theta)\trans\bm Z\trans\bm
    Z\bm\Lambda(\bm\theta)+\bm I_q&\bm\Lambda(\bm\theta)\trans\bm
    Z\trans\bm X\\
    \bm X\trans\bm Z\bm\Lambda(\bm\theta)&\bm X\trans\bm X
  \end{bmatrix}
  \begin{bmatrix}\tilde{\bm u}\\\widehat{\bm\beta}(\bm\theta)\end{bmatrix}
  =
  \begin{bmatrix}
    \bm\Lambda(\bm\theta)\trans\bm Z\trans\bm y\\\bm X\trans\bm y
  \end{bmatrix} ,
\end{equation}
...
factor of $\bm\Lambda(\bm\theta)\trans\bm Z\trans\bm
Z\bm\Lambda(\bm\theta)+\bm I_q$ for many different values of
$\bm\theta$.
...
\begin{equation}
  \label{eq:UV}
  \bm U(\bm\theta)=\bm Z\bm\Lambda(\bm\theta)\bm P\trans\text{ and }
  \bm V=\bm X .
\end{equation}
...
$\widehat{\bm\beta}(\bm\theta)$, of the fixed-effects parameters and
the conditional mean $\bm\mu_{\bc B|\bc Y=\bm y}=\bm\Lambda(\bm\theta)\tilde{\bm
  u}$, of the random effects.  If $r^2$ is the minimum penalized
residual sum of squares,
\begin{equation}
  \label{eq:minpls}
  r^2(\bm\theta)=\|\bm y-\bm U\tilde{\bm u}(\bm\theta)-
  \bm V\widehat{\bm\beta}(\bm\theta)\|^2+\|\tilde{\bm u}(\bm\theta)\|^2
\end{equation}
then the conditional maximum likelihood estimate of $\sigma^2$ is
$\widehat{\sigma^2}(\bm\theta)=\frac{r^2}{n}$ and the conditional REML
estimate of $\sigma^2$ is
$\widehat{\sigma^2}_R(\bm\theta)=\frac{r^2}{n-p}$.

Although we have not written this explicitly, the matrices $\bm L$,
$\bm R_{ZX}$ and $\bm R_X$ all depend on the value of $\bm\theta$.
Both the $q\times q$ matrix $\bm L(\bm\theta)$ and the $p\times p$
matrix $\bm R_X(\bm\theta)$ are triangular: $\bm L(\bm\theta)$ is
lower triangular and $\bm R_x(\bm\theta)$ is upper triangular.
