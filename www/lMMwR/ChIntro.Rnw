...
terms in what follows.  In Chapter~\ref{chap:computational} we revisit
these definitions providing derivations and more explanation.

As mentioned in section~\ref{sec:memod}, a mixed model incorporates
...
\begin{equation}
  r^2(\vec\theta,\vec\beta,\vec u)=\left\{\|\vec y -\vec X\vec\beta -\vec
...
\begin{equation}
  r^2_{\beta,\theta} =\min_{\vec u}\left\{\|\vec y -\vec X\vec\beta -\vec
    Z\Lambda_\theta\vec u\|^2+\|\vec u\|^2\right\}
\end{equation}
is a direct (i.e. non-iterative) computation for which we calculate
...
Because the conditional mean, $\vec\mu$, is a linear function of
...
random effects and the conditional estimate, $\widehat{\beta}_\theta$,
of the fixed effects.  At the conditional estimate of the fixed
effects the deviance is
\begin{equation}
  \label{eq:LMMdeviance}
...
\begin{equation}
  \widehat{\sigma^2}_\theta=r^2_\theta/n 
\end{equation}
...
The \emph{maximum likelihood estimate} (MLE) of $\vec\theta$, written
$\widehat{\vec\theta}$, is the value that minimizes the profiled
deviance.  We determine this value by numerical optimization.  In the
process of evaluating $\tilde{d}((\widehat{\vec\theta}|\vec y)$ we
determine $\widehat{\beta}$, $\tilde{\vec u}_{\widehat{\theta}}$ and
...
BLUPs of the random effects.  Other than having an appealing acronym,
...
section~\ref{sec:assessRE}.

\subsection{Matrices and Vectors in the Fitted Model Object}
\label{sec:FittedModel}

The optional argument, \code{verbose = TRUE}, in a call to \code{lmer}
...
@ 
The algorithm converges in 6 iterations to a profiled deviance of
\Sexpr{sprintf("%.5f", deviance(fm1ML))}
at $\theta=$\Sexpr{sprintf("%.6f", fm1ML@getPars())}.
  
The actual values of many of the matrices and vectors defined above
are available in the \emph{environment} of the fitted model object,
accessed with the \code{env} function.  For example,
$\Lambda_{\widehat{\theta}}$ is
<<fm1MLLambda>>=
env(fm1ML)$Lambda
@ 
Often we will show the structure of sparse matrices as an image
(Figure~\ref{fig:fm1Lambdaimage}).
\begin{figure}[tbp]
  \centering
<<fm1Lambdaimage,fig=TRUE,echo=FALSE,height=2.5>>=
print(image(env(fm1)$Lambda, sub=NULL))
@   
\caption[Image of the $\Lambda$ for model \texttt{fm1ML}]{Image of the
  relative covariance factor, $\Lambda_{\widehat{\theta}}$ for model
  \code{fm1ML}.  The non-zero elements are shown as darkened squares.
  The zero elements are blank.}
  \label{fig:fm1Lambdaimage}
\end{figure}
...
transpose is shown in Figure~\ref{fig:fm1Ztimage}, consists of the
...
multiple of the identity (you can check this with
\code{image(env(fm1ML)\$L}).  The vectors $\vec u$ and $\vec b$ and
the matrix $\vec X$ have the same names in \code{env(fm1ML)}.  The
vector $\beta$ is called \code{fixef}.

In the remainder of this section we discuss the derivation of the
quantities defined above.  Readers who are less interested in the
``why'' than in the ``what'' should feel free to skip to
...
parameters and \emph{profile pairs} plots for parameter pairs.

As in the previous section, we will state the results first and provide more
detailed derivations and explanations later.  
...
confidence interval on the parameter consists of the values for which
$-1.960 < \zeta < 1.960$.
...
the original fit is a REML fit so there is a slight advantage in
...
(Figure~\ref{fig:fm1prof}) are obtained with
<<fm1profshow>>=
...
parameter (Figure~\ref{fig:fm1absprof}), obtained by adding the
...
A profile zeta plot, such as Figure~\ref{fig:fm1prof}, shows us the
...
distribution of an estimator, it is a similar idea and we wil use some
...
normal probability plot of data values or residuals from a model.

Ideally the pattern in the plot will be a straight line, in which case
we can perform statistical inference based on the parameter's
estimate, its standard error and quantiles of the standard normal
distribution.  We will describe such as situation as providing a good
normal approximation for inference.  The common practice of quoting a
parameter estimate and its standard error assumes that this is always
the case.

In Figure~\ref{fig:fm1prof} the profile zeta plot for $\log(\sigma)$
\begin{figure}[tb]
  \centering
...
@   
\caption{Signed square root, $\zeta$, of the likelihood ratio test
  statistic as a function of $\log(\sigma)$, of $\sigma$ and of
  $\sigma^2$.  The vertical lines are the endpoints of 50\%, 80\%,
...
approximation for $\sigma^2$ or even $\sigma$.  As shown in
Figure~\ref{fig:sigmaprof} the profile zeta plot for $\log(\sigma)$ is
...
@   
\caption{Signed square root, $\zeta$, of the likelihood ratio test
  statistic as a function of $\log(\sigma_1)$, of $\sigma_1$ and of
  $\sigma_1^2$.  The vertical lines are the endpoints of 50\%, 80\%,
  90\%, 95\% and 99\% confidence intervals.}
  \label{fig:sigma1prof}
\end{figure}
Figure~\ref{fig:sigma1prof} shows the profile zeta plot on the scale
...
A profiled deviance object, such as \code{pr1} not only provides
...
<<fm1profpairshow,eval=FALSE>>=
splom(pr1)
@
and shown in Figure~\ref{fig:fm1profpair} shows the profile traces
...
what is shown in each panel.  To make it easier to refer to panels we will
...
numbered 1 to 3 from bottom to top.  Note that the rows are
numbered from the bottom to the top, like the y-axis of a graph, not
from top to bottom, like a matrix.
...
is a straight horizontal line, meaning that the conditional estimate
...
quadratic in these parameters.  That is, $\zeta(\beta_0)$ and
$\zeta(\log(\sigma))$ not perfectly linear.
...
axes on each of these panels are the same values, running from $-3$ to
...
In section~\ref{sec:definitions} we mentioned that what are sometimes
called the BLUPs (or best linear unbiased estimators) of the random
effects, $\bc B$, are the conditional modes evaluated at the parameter
estimates, and that they can be calculated as
$\tilde{b}_{\widehat{\theta}}=\Lambda_{\widehat{\theta}}\tilde{u}_{\widehat{\theta}}$.

These values are often considered as some sort of ``estimates'' of the
random effects and that can be helpful but it can also be misleading.
As we have stated several times, the random effects are not, strictly
speaking, parameters; they are unobserved random variables.  We don't
estimate the random effects the way that we estimate parameters.
Instead, we consider the conditional distribution of $\bc B$ given the
observed data.  That is, we consider $(\bc B|\bc Y=\vec y)$.

Because the unconditional distribution, $\bc B\sim\mathcal{N}(\vec
0,\Sigma_\theta$ is continuous, the conditional distribution, $(\bc
B|\bc Y=\vec y)$ will also be continuous.  The phrase ``conditional
mode'' refers to the point at which the conditional density of $\bc
B$, given $\bc Y=\vec y$, is maximized.  Because this definition
relates to the probability model, the values of the parameters are
assumed to be known.  In practice, of course, we don't know the values
of the parameters (if we did there would be no purpose in forming the
parameter estimates), so we use the estimated values of the
...
which is the conditional mean of $\bc Y$ given $\bc B$, and is an
important part of all forms of mixed-effects models.
...
str(ranef(fm1ML))
@
shows that the values is a list of data frames.  In this case the
...
described by this term.  An expression for the grouping factor,
...
elements of the list are associated with grouping factors, not the
terms.

Given the data $\bc y$ and the parameter estimates we can evaluate a
...
\begin{figure}[tbp]
  \centering
<<fm1preddot,fig=TRUE,echo=FALSE,results=hide,height=2.5>>=
print(dotplot(ranef(fm1ML, postVar=TRUE)))
@   
\caption{95\% prediction intervals on the random effects in \code{fm1ML}}
  \label{fig:fm1preddot}
\end{figure}

We can plot these prediction intervals using
<<fm1preddotshow,eval=FALSE>>=
dotplot(ranef(fm1ML, postVar = TRUE))
@ 
(Figure~\ref{fm1preddot}), which provides linear spacing
\begin{figure}[tbp]
  \centering
<<fm1predqq,fig=TRUE,echo=FALSE,results=hide,height=3.5>>=
print(qqmath(ranef(fm1ML, postVar=TRUE)))
@   
\caption{95\% prediction intervals on the random effects in \code{fm1ML}}
  \label{fig:fm1predqq}
\end{figure}
of the levels on the y axis, or using
<<fm1predqqshow,eval=FALSE>>=
qqmath(ranef(fm1ML, postVar=TRUE))
@
(Figure~\ref{fig:fm1predqq}), where the intervals are plotted versus
quantiles of the standard normal.

The dotplot is preferred when there are only a few random effects per
term, as in this case.  When there are hundreds or thousands of random
effects the \code{qqmath} form is preferred because it focuses
attention on the ``important few'' at the extremes and de-emphasizes
the ``trivial many'' that are close to zero.
...
model that is simple, not the material).  It is worthwhile summarizing
...
Gaussian distributions.  Furthermode, this conditional distribution is
...
In the models we considered in this chapter, $\Sigma_\theta$, was a
simple multiple of the identity matrix, $\vec I_6$.  This will always
be the case for models with just one random-effects term that is a
simple, scalar term.  The reason for introducing all this machinery is
to allow for more general models.

The maximum likelihood estimates of the parameters are obtained by
minimizing the deviance.  For these models we can minimize the
