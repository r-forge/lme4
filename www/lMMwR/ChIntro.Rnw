...
%% FIXME: This paragraph needs revision
For models fit by the REML criterion the only statistic describing the
model fit is the value of the REML criterion itself.  To obtain the
maximum likelihood estimates instead we provide the optional argument
\code{REML = FALSE} estimate the parameters by maximum likelihood,
<<fm1ML>>=
...
estimates.  These are all statistics related to the model fit and they
are primarily used to compare different models fit to the same data.
We discuss them in more detail in section~\ref{sec:logLik}.
...
factor of the variance, $\bm\Sigma_\theta$, of $\bc B$ relative to the
...
I_6)=\theta\bm Z$ and $\bm U_\theta\trans\bm U_\theta+\bm I_6=(1 + 5\theta^2)\bm
I_6$ so a suitable $\bm L_\theta=\sqrt{1 + 5\theta^2}\bm I_6$.  The
...
\begin{equation}
  \label{eq:PLRSS}
  r^2_\theta = \min_{\bm u,\bm\beta}\|\bm y-\bm U_\theta\bm u-\bm
  X\bm\beta\|^2+\|\bm u\|^2
\end{equation}
...
\begin{equation}
  \label{eq:profDev}
  -2\tilde{\ell}(\bm\theta)=\log(|\bm L|^2)+n\left[1+
...
U_\theta\tilde{\bm u}(\theta)-\bm X\widehat{\bm\beta}_\theta\|^2$
...
labeled \code{usqr} in  Figure~\ref{fig:prssCom}.  It is zero when
