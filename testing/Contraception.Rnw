...
\SweaveOpts{
  ...
  fig.align=center,dev=pdf,tidy=FALSE,keep.source=TRUE,echo=FALSE,
fig.path=figures/Contraception-}
...
(This plot has become quite unwieldy since I generated a
plot with more detail.  Need a better way to display
it if we want to bother.)
...
LL2 <- lapply(LL,function(x) {
  names(x)[grep("^VarCorr",names(x))] <- "VarCorr.district.urban"
  x
})
@ 
<<eval=FALSE>>=
...
m2 <- melt(subset(a2,
                  select=c(pkg,method,nAGQ,deviance,
                    ...
                    fixef.I.age.2.,
                    fixef.chY,
                    VarCorr.district.urban)),
           id.var=c("pkg","method","nAGQ"))
@ 

<<allplot,warning=FALSE>>=
(gg2 <- ggplot(m2,aes(x=factor(nAGQ),y=value,colour=droplevels(interaction(pkg,method))))+
...
Without \code{lme4}:
<<Contraception-allplot2,warning=FALSE>>=
gg2 %+% subset(m2,pkg!="lme4")
@ 
