...
dfun_lme4_bobyqa <- function(rb,be,tolpwrss,nAGQ=1,...,problem,debug=FALSE) {
  if (debug) cat(rb,be,tolpwrss,"\n")
  control <- list(rhobeg=rb,rhoend=rb*be)
  d0 <- list(problem=problem,pkg="lme4",method="bobyqa",optimx=TRUE,nAGQ=nAGQ,
...
dfun_lme4_NM <- function(tolpwrss,nAGQ,family,
                              ...,problem,debug=FALSE) {
  d0 <- list(problem=problem,pkg="lme4",method="Nelder_Mead",
             optimx=FALSE,nAGQ=nAGQ,
  ...
  if (!is.na(nAGQ)) {
    ...
    t0 <- system.time(cur.opt <- try(lmer(...,
                                          optimizer="Nelder_Mead"),
                                     silent=TRUE))
  }
  if (inherits(cur.opt,"try-error")) {
