...
<<datainput>>=
str(orig <- read.delim(unz(system.file("original/text-star.zip", package = "mlmRev"),
                           "webstar.txt"), colCl = rep(c("factor", "integer"), c(5, 48))))
@ 
...
load(system.file("original/orig.rda", package = "mlmRev"))
@
