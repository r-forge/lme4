...
//    void reModule::updateDcmp(Rcpp::NumericVector &cmp) const {  // Need Matrix_0.999375-42 or later
	...
	size_t nf = d_flist.size();
	IntegerVector ans(nf);
	for (size_t i = 0; i < nf; ++i) {
	    IntegerVector fli = d_flist[i];
	    CharacterVector levs = fli.attr("levels");
	    ans[i] = levs.size();
	}
	return ans;
    }
    ...
    Rcpp::IntegerVector reTrms::terms(int ii) const {
	int ip1 = ii + 1;   // for matching 1-based indices in d_assign
