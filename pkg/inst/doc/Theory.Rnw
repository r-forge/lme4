...
of the parameters given the observed data, for maxmimum likelihood
...
different parameter values during the optimization process, we focus
on the evaluation of the objective function and the critical
computation in the this evalution, which is determining the solution
to a penalized, weighted least squares (PWLS) problem.
...
In the next section we describe a particular form of mixed model,
called a linear mixed model, and also the general form of the mixed models
that can be represented in the \package{lme4} package.
...
representing a count. In our formulation, the random variable $\bm B$
is always continous.

We specify a mixed model by describing the unconditional distribution
of $\bc B$ and the conditional distribution $\bc Y|\bc B$.
...
understanding that, in some models, $\sigma=1$ by definition.
...
$\bm\theta$.
%% FIXME: check how to write "much less than"
Typically $m<q$; in the examples we show below it is always the
case that $m<5$, even when $q$ is in the thousands.  The fact that $m$
is very small is important because, as we shall see, determining the
parameter estimates in a mixed model can be expressed as an
optimization problem with respect to $\bm\theta$ only.

The parameter $\bm\theta$ may be, and typically is, subject to
constraints. For ease of computation, we require that these can be
expressed as ``box'' constraints of the form
$\theta_{iL}\le\theta_i\le\theta_{iU},i=1,\dots,m$ for constants
$\theta_{iL}$ and $\theta_{iU}, i=1,\dots,m$.  We shall write such
constraints as $\bm\theta_L\le\bm\theta\le\bm\theta_R$.  The matrix
...
The conditional distribution, $(\bc Y|\bc B=\bm b)$, must satisfy the
...
\item The conditional mean, $\bm\mu_{\bc Y|\bc B}(\bm b) =
  ...
  $q$ and $p$ columns, respectively.  Frequently, the number of rows
  in $\bm Z$ and $\bm X$ is $n$, the dimension of $\bm y$, but not always.
\item The scalar distributions, $(\mathcal{Y}_i|\bc B=\bm
  b),i=1,\dots,n$, are independent.  We say that the components of
  $\bc Y$ are \emph{conditionally independent} given $\bc B$.
  Furthermore, the scalar distributions must all have the same form.
  They differ only in the value of the conditional means, $\mu_i(\bm b)$.
\item The scalar distributions, $(\mathcal{Y}_i|\bc B=\bm
  b),i=1,\dots,n$, are completely determined by the conditional mean,
  $\bm\mu_{\bc Y|\bc B}(\bm b)$ and, at most, one additional
  parameter, $\sigma$, which is the common scale parameter.
\end{enumerate}
...
$\bm b$ only through the linear predictor, we can express the model in
terms of a linear transformation of $\bc B$.  We define the linear
transformation from a $q$-dimensional ``spherical'' Gaussian random variable,
$\bc U$, to $\bc B$ as
\begin{equation}
  \label{eq:3}
...
probability density for $\bc U$ are spheres centered at the mean - in
this case, $\bm0$.)

When $\bm\theta$ is not on the boundary this is an invertible
transformation but we will only need to express $\bc B$ in terms of
$\bc U$ and that transformation is defined even when $\bm\theta$ is on
the boundary.
...
use the notation $\bm\gamma(\bm u,\bm\theta,\bm\beta)$.

\section{The conditional density $(\bc U|\bc Y=\bm y)$}
\label{sec:cond-dens-bc}

Because we observe $\bm y$ and do not observe $\bm b$, the conditional
distribution of interest, for the purposes of statistical inference,
is $(\bc U|\bc Y=\bm y)$ (or $(\bc B|\bc Y=\bm y)$ but, if we know
$(\bc U|\bc Y=\bm y)$, it is staightforward to derive $(\bc B|\bc
Y=\bm y)$.) This conditional distribution is always a continuous
distribution with a well-defined density.

We can evaluate the conditional density, $f_{\bc U|\bc Y}(\bm u|\bm
y)$, up to a constant, as the product of the unconditional density,
$f_{\bc U}(\bm u)$, and the conditional density (or the probability mass
function, whichever is appropriate), $f_{\bc Y|\bc U}(\bm y|\bm u)$.
We write this unnormalized conditional density as
\begin{equation}
  \label{eq:5}
...
$\sigma$, given the observed data $\bm y$.  The \emph{maximum
  likelihood estimates} of these parameters are the values that
maximize $L$.

\section{The PWLS problem}
...
first considering computational methods for linear mixed models.

Recall from \S\ref{sec:cond-distr-YB} that, in a linear mixed model,
both the conditional distribution, $(\bc Y|\bc U=\bm u)$, and the
unconditional distribution, $\bc U$, are spherical Gaussian distributions
and that the conditional mean, $\bm\mu_{\bc Y|\bc U}(\bm u)$, is the
linear predictor, $\bm\gamma(\bm u)$.  It is convenient to express all
of these densities on the scale of the logarithm of the density or, as
is common in many statistical expressions, on the \emph{deviance
  scale} (negative twice the logarithm of the density),
\begin{equation}
  \label{eq:7}
  \begin{aligned}
    -2\log(f_{\bc U}(\bm u))&=q\log(2\pi\sigma)+\frac{\|\bm u\|^2}{\sigma^2}\\
    -2\log(f_{\bc Y|\bc U}(\bm y|\bm u))&=n\log(2\pi\sigma)+
    \frac{\|\bm y-\bm Z\bm\Lambda(\bm\theta)\bm u-\bm X\bm\beta\|^2}{\sigma^2}\\    
    -2\log(h(\bm u|\bm y,\bm\theta,\bm\beta,\sigma))
    &= (n+q)\log(2\pi\sigma)+
    \frac{\|\bm y-\bm Z\bm\Lambda(\bm\theta)\bm u-\bm X\bm\beta\|^2+\|\bm
      u\|^2}{\sigma^2}\\
    &= (n+q)\log(2\pi\sigma)+
...
\begin{equation}
  \label{eq:9}
    ...
    \|\bm y-\bm Z\bm\Lambda(\bm\theta)\bm u-\bm X\bm\beta\|^2+\|\bm u\|^2
\end{equation}
has the form of a penalized residual sum of squares in that the first
term, $\|\bm y-\bm Z\bm\Lambda(\bm\theta)\bm u-\bm X\bm\beta\|^2$ is
...
Using a so-called ``pseudo data'' representation, we can
write the discrepancy as a residual sum of squares from a model that
...
linear predictor, $\bm Z\bm\Lambda(\bm\theta)\bm u+\bm X\bm\beta$, in
such a way that their contribution to the overall residual sum of
squares is exactly the penalty term in the discrepancy.
...
\begin{equation}
  ...
  \begin{bmatrix}
    ...
    +\bm I_q&\bm\Lambda\trans(\bm\theta)\bm Z\bm X\\
...
Cholesky decomposition with fill-reducing permutation $\bm P$ is the
...
\end{multline}
The diagonal elements of $\bm L_{\bm Z}$ and $\bm L_{\bm X}$ are only
known up to a change in sign.  By convention we choose them to be
positive so that the determinants $|\bm L_{\bm Z}|$ and $|\bm L_{\bm
  X}|$ are positive.  These determinants are easily calculated as the
...
\begin{equation}
  ...
  \left\|
    \begin{bmatrix}
      ...
      \bm 0&\bm L_{\bm X}
    \end{bmatrix}
    \begin{bmatrix}\bm u-\tilde{\bm u}\\\bm\beta-\tilde{\bm\beta}
    \end{bmatrix}
  \right\|^2
\end{equation}
where
\begin{equation}
  \label{eq:18}
  \tilde{d}(\bm y,\bm\theta)=
  d(\tilde{\bm u}|\bm y,\bm\theta,\tilde{\bm\theta})
\end{equation}
from which we can evaluate the \emph{profiled likelihood}, a function
of $\bm\theta$ only.  On the deviance scale the profiled likelihood is
\begin{equation}
  \label{eq:19}
  -2\log(\tilde{L}(\bm\theta|\bm y)=
...
In practice the restricted or residual maximum likelihood estimates of
variance components are often preferred to the maximum likelihood
estimates.  For example, in a linear regression model
\begin{equation}
  \label{eq:20}
  \bc Y\sim\mathcal{N}(\bm X\bm\beta,\sigma^2\bm I_n)
\end{equation}
we typically use the estimate
\begin{equation}
  \label{eq:21}
  \widehat{\sigma^2}=\frac{\|\bm y-\bm X\widehat{\bm\beta}\|^2}{n-p}
\end{equation}
...
Although not the REML criterion is not usually derived in this way, it
can be expressed as
\begin{equation}
  \label{eq:23}
  d_R(\bm\theta,\bm\sigma|\bm y)=-2\log
  \int_{\mathbb{R}^p}L(\bm u|\bm y,\bm\theta,\bm\beta,\sigma)\,d\bm\beta
\end{equation}
on the deviance scale.  The profiled REML criterion is
\begin{equation}
  \label{eq:24}
  \tilde{d}_R(\bm\theta|\bm y)=
  \log(|\bm L_{\bm Z}(\bm\theta)|^2)+
  \log(|\bm L_{\bm X}(\bm\theta)|^2)+(n-p)
...
% variance of the random noise terms in an
% ordinary least squares regression model is 

% is the discrepancy evaluated at 
% For linear mixed models the discrepancy is a penalized residual sum of
% squares, which is a positive definite quadratic form in $\bm u$ and
% also in $\bm\beta$. In the next section we show that we can reexpress
% this quadratic form in the canonical form
% \begin{equation}
%   \label{eq:12}
%   d(\bm u|\bm y,\bm\theta\bm\beta) =
%   d(\tilde{\bm u}|\bm y,\bm\theta,\widehat{\bm\beta}) +
%   \begin{bmatrix}\bm u-\tilde{\bm u}\\
%     \bm\beta -\widehat{\bm\beta}\end{bmatrix}\trans
%   \begin{bmatrix}
%     \bm L_{\bm Z}&\bm 0\\\bm L_{\bm{XZ}}&\bm L_{\bm X}
%   \end{bmatrix}
%   \begin{bmatrix}
%     \bm L_{\bm Z}&\bm 0\\\bm L_{\bm{XZ}}&\bm L_{\bm X}
%   \end{bmatrix}\trans
%   \begin{bmatrix}\bm u-\tilde{\bm u}\\
%     \bm\beta -\widehat{\bm\beta}\end{bmatrix}
% \end{equation}
% where

% Our general approach to evaluating the likelihood,
% $L(\bm\theta,\bm\beta,\sigma|\bm y)$, defined in (\ref{eq:6}), is
% first to determine the mode, $\tilde{\bm u}(\bm
% y|\bm\theta,\bm\beta)$, of $(\bc U|\bc Y=\bm y)$, which is the value
% of $\bm u$ that that maximizes the conditional density.  This would be
% rather a pointless definition (we need to evaluate the likelihood
% before we can evaluate the conditional density) were it not for the
% fact that the conditional mode also maximizes the
% unnormalized
% conditional density, $h(\bm u|\bm y,\bm\theta,\bm\beta,\sigma)$ and
% minimizes the discrepancy, $d(\bm u|\bm y,\bm\theta,\bm\beta)$.  That is
% \begin{equation}
%   \label{eq:11}
%   \begin{aligned}
%     \tilde{\bm u}(\bm y|\bm\theta,\bm\beta)
%     &=\arg\max_{\bm u}h(\bm u|\bm y,\bm\theta,\bm\beta,\sigma)\\
%     &=\arg\min_{\bm u}d(\bm u|\bm y,\bm\theta,\bm\beta) .
%   \end{aligned}
% \end{equation}

% As we shall see later, for
% other types of mixed models the problem of determining the conditional
% mode can be expressed as a weighted penalized least squares problem,
% where the weights depend on the value of $\bm u$.  In general we will
% use penalized iteratively reweighted least squares (PIRLS) to
% determine the conditional mode.

% Because In both the
% At the level of the PLS or PIRLS problem, the r
% For linear mixed models the discrepancy is a quadratic form in both
% $\bm u$ and $\bm\beta$.  
% \subsection{Matrix decompositions for solving PLS problems}
% \label{sec:matr-decomp-solv}
...
\package{lme4} package, consists of one or more random-effects terms and
