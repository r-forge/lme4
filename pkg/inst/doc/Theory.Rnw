...
Typically $m\ll q$; in the examples we show below it is always the
...
transformation can fail to be invertible.  However, we will only
require that we be able to express $\bc B$ in terms of $\bc U$ and
that transformation is well-defined, even when $\bm\theta$ is on the
boundary.
...
distribution, defined by the conditional probability density, $f_{\bc
  U|\bc Y}(\bm u|\bm y)$.
...
consider their densities.  It is convenient to express these densities
on the scale of the logarithm of the density or, as is common in many
statistical expressions, on the \emph{deviance scale} (negative twice
...
\end{multline}
The diagonal elements of $\bm L_{\bm Z}$ and $\bm L_{\bm X}$ are
determined only up to a change in sign.  By convention we choose them
to be positive. 
...
\subsection{Profiled log-likelihood for linear mixed models}
...
\begin{multline}
  ...
  \right\|^2}{\sigma^2} .
\end{multline}
We need to integrate $h$ with respect to $\bm u$ and minimize the
resulting function with respect to the parameter values.
We first minimize $h$ with respect to $\bm\beta$ by setting
$\bm\beta=\tilde{\bm\beta}(\bm\theta)$.
...
\end{equation}
and the maximum likelihood estimate of $\bm\theta$ is
\begin{equation}
  \label{eq:29}
  \widehat{\bm\theta}_L=\arg\min_{\bm\theta}
  \left(-2\log(\tilde{L}(\bm\theta|\bm y))\right) .
\end{equation}

The maximum likelihood estimates of $\sigma^2$ and $\bm\beta$ are then
evaluated as
\begin{align}
  \label{eq:30}
  \widehat{\sigma^2_L}&=
...
often preferred to the maximum likelihood estimates.  (``REML'' is an
acronym for ``restricted'' or ``residual'' maximum likelihood,
although neither term is completely accurate because these estimates
do not maximize a likelihood.) We can motivate the use
of the REML criterion by considering a linear regression model,
\begin{equation}
  \label{eq:20}
...
We determine the maximum likelihood (ML) or REML estimates of the
parameters $\bm\theta$, $\bm\beta$ and $\sigma$ in a linear mixed
model by first minimizing the profiled log-likelihood or the profiled REML
criterion with respect to $\bm\theta$.  Thus the first implementation
question is evaluating the profiled log-likelihood or the profiled
REML criterion.
