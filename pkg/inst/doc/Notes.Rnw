...
For our purposes we may assume a ``spherical'' multivariate normal
...
matrix, $\sigma^2\bm\Sigma(\bm\theta)$.  The scalar, $\sigma^2$, is
the same as in variance-covariance of the conditional distribution of
$\bm{\mathcal{Y}}$.  As described in the next section, the relative
variance-covariance matrix, $\bm\Sigma(\bm\theta)$, is a $q\times q$
positive semidefinite matrix depending on a parameter vector,
...
of the levels of one or more factors.  Hence, it is a \emph{sparse
  matrix}, in the sense that most of the elements of $\bm Z$ are zero.
The matrix $\bm A$ is also sparse.  In fact, the structure of $\bm T$
and $\bm S$ are such that $\bm A$ has the same pattern of nonzeros as
$\bm Z\trans$ does.

\subsection{Sparse matrix methods}
\label{sec:sparseMatrix}

The reason for defining $\bm A$ as the transpose of a model matrix
is because $\bm A$ is stored and manipulated as a sparse matrix.  In
the compressed column-oriented storage form for sparse matrices there
are advantages to storing $\bm A$ as a matrix of $n$ columns and $q$
rows.  In particular, the CHOLMOD sparse matrix library allows us to
evaluate the sparse Cholesky factor, $\bm L$, a lower triangular
...
nonzeros in the sparse model matrix $\bm Z$.  $\bm P$ does not affect
the statistical theory (if $\bm{\mathcal{U}}\sim\mathcal{N}(\bm
0,\sigma^2\bm I)$ then $\bm P\trans\bm{\mathcal{U}}$ also has a
$\mathcal{N}(\bm 0,\sigma^2\bm I)$ distribution because $\bm P\bm
P\trans=\bm P\trans\bm P=\bm I$) but, because it affects the number of
nonzeros in $\bm L$, it can have a tremendous impact on the amount
storage required for $\bm L$ and the time required to evaluate $\bm L$
from $\bm A$.  Indeed, it is precisely because $\bm L(\bm\theta)$ can
be evaluated quickly, even for complex models applied the large data
sets, that the \code{lmer} function is effective in fitting such
models.
...
representation of a mixed-effects model.  Several slots of this class
...
to other methods for linear mixed models, these results apply equally
well to models where $\bm Z$ is derived from multiple grouping factors
that can be crossed or partially crossed in addition to models with
multiple, nested grouping factors.

The system (\ref{eq:PLSsol}) is similar to Henderson's ``mixed-model
equations''.  One important difference is that Henderson represented
his system of equations in terms of $\bm\Sigma^{-1}$ and, in important
practical examples, $\bm\Sigma^{-1}$ does not exist at the parameter
estimates.  Also, Henderson assumed that equations like
(\ref{eq:PLSsol}) would need to be solved explicitly and, as we have seen,
only the decomposition of the system matrix is needed for evaluation
of the profiled log-likelihood.  The same is true of the profiled the
logarithm of the REML criterion, which we define later.
...
sometimes orders of magnitude smaller.  Blithely writing a formula in
