...
for (i in seq(along = sg)) {
    ...
    vals[i,] <- if (inherits(res, "try-error")) NA else mm@deviance
}
...
print(xyplot(y ~ x | type, df, type = c("g", "l"),
             subset = !(type %in% c("REML", "ldRX2", "usqr", "disc", "wrss")),
...
print(image(tcrossprod(sm1@Cm), colorkey = FALSE, aspect = 1),
...
print(image(tcrossprod(Om1@Cm), colorkey = FALSE, aspect = 1),
       split = c(1,1,2,1), more = TRUE)
...
print(image(tcrossprod(Om1a@Cm), colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
...
print(image(tcrossprod(sm2@Cm), colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
...
print(image(tcrossprod(Om2@Cm), colorkey = FALSE, aspect = 1),
      split = c(1,1,2,1), more = TRUE)
...
<<Sm1fill>>=
c(A = length(tcrossprod(Sm1@Cm)@x), L = length(Sm1@L@x))
@
