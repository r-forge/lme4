...
stopifnot(suppressPackageStartupMessages(require(lme4b)))
...
(fmX2 <- lmer1(Reaction ~ Days + (Days|Subject), sleepstudy))
(fm.2 <- lmer1(Reaction ~ Days + (1|Subject) + (0+Days|Subject), sleepstudy))

fmX1s <- lmer1(Reaction ~ Days + (Days|Subject), sleepstudy, sparseX=TRUE)
fmX2s <- lmer1(Reaction ~ Days + (Days|Subject), sleepstudy, sparseX=TRUE)
...
fm2l <- lmer1(log(Reaction) ~ log(Days+1) + (log(Days+1)|Subject),
...
#m1 <- glmer1(cbind(incidence, size - incidence) ~ period + (1 | herd),
...
if (FALSE) {
    if (require('MASS', quietly = TRUE)) {
        ...
        print(fm5 <- glmer1(y ~ trt + wk2 + (1|ID), bacteria, binomial, verbose = TRUE))
...
try(reg <- lmer1(y ~ habitat + (1|habitat*lagoon), data = dat) # did seg.fault
    ) # now gives error                 ^- should be ":"
r1  <- lmer1(y ~ 0+habitat + (1|habitat:lagoon), data = dat) # ok, but senseless
r1b <- lmer1(y ~ 0+habitat + (1|habitat), data = dat) # same model, clearly indeterminable
...
r2.0 <- lmer1(y ~ 0+lagoon + (1|habitat:lagoon), data = dat) # ok
r2   <- lmer1(y ~ 0+lagoon + (1|habitat), data = dat) # ok, and more clear
...
r2.  <- lmer1(y ~ 0+lagoon + (1|habitat), data = dat,
...
tc <- tryCatch(
	       m2 <- glmer1(incidence / size ~ period, weights = size,
...
## --> do need CPU
m0 <- lmer1(y ~ (x1 + x2)|ff, data = D)
m1 <- lmer1(y ~ x1 + x2|ff  , data = D)
m2 <- lmer1(y ~ x1 + (x2|ff), data = D)
m3 <- lmer1(y ~ (x2|ff) + x1, data = D)
stopifnot(identical(ranef(m0), ranef(m1)),
          identical(ranef(m2), ranef(m3)),
          inherits(tryCatch(lmer1(y ~ x2|ff + x1, data = D), error = function(e)e),
                   "error"))
...
dd <- rPoisGLMMi(12, 20)
m0  <- glmer1(y~x + (1|f),           family="poisson", data=dd)
(m1 <- glmer1(y~x + (1|f) + (1|obs), family="poisson", data=dd))
#anova(m0, m1)
